...
func TestSessionHookOmitsMemoryRecallButPreservesTaskRecallWhenMemoryModuleIsDisabled(t *testing.T) {
	projectDir := t.TempDir()
	lf := &hub.Lockfile{
		Artifacts: map[hub.ArtifactType]map[string]*hub.LockfileArtifactMeta{},
	...
	got := output.String()
	if strings.Contains(got, "graphit_memory_") || strings.Contains(got, "graphit-memory`") {
		t.Fatalf("disabled memory module leaked into hook context: %s", got)
	}
	for _, want := range []string{"Graphit session bootstrap", "graphit_task_search", "graphit_task_get"} {
		if !strings.Contains(got, want) {
			t.Fatalf("enabled task recall missing %q when memory is disabled: %s", want, got)
		}
	}
}

func TestSessionHookOmitsTaskRecallButPreservesMemoryRecallWhenTaskModuleIsDisabled(t *testing.T) {
	projectDir := t.TempDir()
	lf := &hub.Lockfile{
		Artifacts: map[hub.ArtifactType]map[string]*hub.LockfileArtifactMeta{},
	...
	got := output.String()
	if strings.Contains(got, "graphit_task_") || strings.Contains(got, "graphit-task`") {
		t.Fatalf("disabled task module leaked into hook context: %s", got)
	}
	for _, want := range []string{"Graphit session bootstrap", "graphit_memory_search", "graphit_memory_source"} {
		if !strings.Contains(got, want) {
			t.Fatalf("enabled memory recall missing %q when task is disabled: %s", want, got)
		}
	}
}
