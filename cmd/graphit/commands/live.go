...
Examples:
  ...
  ` + brand.BinName() + ` live "where is the parser?" -a ast:acme-graph --agent cursor
  ...
  ` + brand.BinName() + ` live remove 01ARZ3NDEKTSV4RRFFQ69G5FAV`,
	...
	cmd.Flags().StringVar(&agent, "agent", "",
		"Agent conventions for the throwaway project (default: your configured Agent)")
...
func runLive(ctx context.Context, opts liveOptions) error {
	if nonInteractive && strings.TrimSpace(opts.question) == "" {
		return errors.New("live search requires --question with --non-interactive")
	}
	...
	if agentName == "" {
		agentName = config.ResolveAgent("", nil, nil)
	}
	if err := prep.ValidateAgent(agentName); err != nil {
		return err
	}
