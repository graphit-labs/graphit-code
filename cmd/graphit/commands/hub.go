  ...
  ` + brand.BinName() + ` hub install my-skill --alias my-custom-name`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			for _, id := range args {
				...
				result, err := svc.Install(ctx, id, alias, agent, hub.ArtifactType(artType), "", "")
				if err != nil {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			for _, id := range args {
				...
				if err := svc.Uninstall(ctx, id, hub.ArtifactType(artType), true, agent, ""); err != nil {
  ...
  graphit hub update my-rule --type rule`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if len(args) > 0 {
				...
				if err := svc.UpdateOne(ctx, id, hub.ArtifactType(artType), agent, ""); err != nil {
				...
				results := svc.UpdateAll(ctx, agent, "")
				if len(results) == 0 {
...
func newHubSubmitCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err := hubSvc.RecordPublish(ctx, entryID, hub.ArtifactType(artType), version, agent, ""); err != nil {
...
func newHubTypePathCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Short: "Print the Agent path for a physical artifact",
		Long: `Output the absolute path where an artifact should be created
for the current Agent and project.
...
path. Rules have no Agent path because lifecycle hooks load them dynamically.

The output is a single clean path with no extra formatting.`,
		Args: cobra.ExactArgs(2),
		Example: `  ` + binName + ` hub type-path skill my-error-patterns
  ` + binName + ` hub type-path command pre-deploy-check --agent cursor`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			typePath, err := agentAdapter.ArtifactTypePath(wd, agent, artType, artName)
			if err != nil {
				return err
			}
...
hooks from a directory containing RULE.md; they are never copied into an Agent.
...
Agent types:     link the adapter-native target to the source artifact
MCP:           synchronize the source definition into the active adapter`,
  ...
  ` + brand.BinName() + ` hub link error-patterns --path ../my-project --type skill`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			result, err := svc.Link(ctx, args[0], sourcePath, agent, hub.ArtifactType(artType), "")
			if err != nil {
...
func newHubUnlinkCmd() *cobra.Command {
  ...
  ` + brand.BinName() + ` hub unlink error-patterns --type skill`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err := svc.Unlink(ctx, args[0], agent, hub.ArtifactType(artType), ""); err != nil {
