...
func TestConfigRejectsRetiredAuthenticationKeys(t *testing.T) {
	_, cleanup := setupTestHome(t)
	defer cleanup()

	if _, err := executeCommand("config", "--global", "hub.secret_access_key", "never-print-this"); err == nil || !strings.Contains(err.Error(), "authentication state") {
		t.Fatalf("retired authentication key was accepted: %v", err)
	}
}
