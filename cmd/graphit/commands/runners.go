...
func refreshModuleSkill(module, projectDir, agentName string) {
	if projectDir == "" {
	...
	projectCfg, lockfileAgents := loadProjectLockInfoFromDir(projectDir)

	if agentName == "" {
		agentName = config.ResolveProjectAgent("", nil, projectCfg, lockfileAgents)
	}

	var err error
	switch module {
	case "knowledge":
		err = knowledge.InstallSkill(projectDir, agentName)
	case "ast":
		err = ast.InstallSkill(projectDir, agentName)
	case "memory":
		err = memory.InstallSkill(projectDir, agentName)
	case "hub":
		err = hub.InstallSkill(projectDir, agentName)
	case "task":
		err = graphtask.InstallSkill(projectDir, agentName)
	}
	_ = err
}
...
func runASTIndex(targetPaths []string, workers int, reset bool, reindex bool, cluster string, clusterPaths []string, noSource bool, grammar string) error {
	...
	if reset {
		...
		p.Step("Waiting for active embedding work...")
		lockedCtx, lifecycleLock, err := storelifecycle.Acquire(ctx, storeDir)
		if err != nil {
			return fmt.Errorf("locking AST store for reset: %w", err)
		}
		...
		if err := os.RemoveAll(storeDir); err != nil {
			return fmt.Errorf("resetting AST store: %w", err)
		}
...
func runASTWatch(targetPath string, workers int, cluster string, clusterPaths []string) error {
	...
	cfg.StoreDir = ast.DefaultLadybugConfig().StoreDir
...
func runUnifiedServe(repoPath string) error {
	...
	agent := os.Getenv(brand.EnvVar("AGENT"))
	if agent == "" {
	...
	srv, err := uiserver.NewUnifiedServer(hubSvc, agent, astDB, repoPath, projectName)
	if err != nil {
		return err
	}
...
func runASTImport(sourcePath, name string, reset bool, workers int) error {
	...
	if reset {
		...
		if lockErr != nil {
			return fmt.Errorf("locking AST context store for reset: %w", lockErr)
		}
		defer lifecycleLock.Release()
		ctx = lockedCtx
		...
		if err := os.RemoveAll(ictx.StoreDir); err != nil {
			return fmt.Errorf("resetting AST context store: %w", err)
		}
...

	{
		...
		if msErr == nil {
			...
			if err := memsvc.IndexMemories(context.Background()); err != nil {
				p.StepWarn("Memory context %q: %v", name, err)
			} else {
				p.Step("Memory context indexes ready at %s", memsvc.TableURI())
			}
			_ = memsvc.Close()
		}
	}
...
func runASTClean(contextName string) error {
	...
	lockedCtx, lifecycleLock, err := storelifecycle.Acquire(ctx, storeDir)
	if err != nil {
		return fmt.Errorf("locking AST store for clean: %w", err)
	}
	defer lifecycleLock.Release()
	ctx = lockedCtx
...
func runKnowledgeClean() error {
	...
	_, lifecycleLock, err := storelifecycle.Acquire(context.Background(), wikiDir)
	if err != nil {
		return fmt.Errorf("locking knowledge store for clean: %w", err)
	}
	defer lifecycleLock.Release()
...
func runMemorySearch(term string, userScope bool) error {
	...
	defer func() { _ = svc.Close() }()
	results, err := svc.SearchMemories(ctx, term, 0, memory.SearchOptions{})
	if err != nil {
		return err
	}
	if len(results) == 0 {
	...
	for _, r := range results {
		...
		priority := memory.PriorityLabel(r.Mandatory, r.Important)
		if r.Superseded {
			p.ListItem("[%s, %.3f] %s — %s (superseded — current: %s)", priority, r.Score, slug, r.Title, r.Current)
			continue
		}
		p.ListItem("[%s, %.3f] %s — %s", priority, r.Score, slug, r.Title)
	}
	p.Count("match", len(results))
	return nil
}
...
func runMemoryList(userScope bool) error {
	...
	for _, m := range memories {
		...
		p.ListItem("[%s] %s [%s]%s", m.ID, m.Title, memory.PriorityLabel(m.Mandatory, m.Important), ts)
	}
...
func runMemoryIndex(userScope, reset bool) error {
	...
	if reset {
		p.Step("The authoritative table has no derived index to clear; refreshing its indexes in place.")
	}
	task := p.StartTask("Refreshing memory indexes at %s...", svc.TableURI())
	if err := svc.IndexMemories(ctx); err != nil {
		task.Fail("Memory indexing failed: %v", err)
		return err
	}
	task.Done("Authoritative memory indexes are current")
	return nil
}
...
func runMemoryQuery(query string, userScope bool, contextName string) error {
	...
	p.Running("Searching authoritative memory table [%s]…", scopeLabel)
	results, err := svc.SearchMemories(ctx, query, 20, memory.SearchOptions{})
	if err != nil {
		return err
	}
	var contextBuilder strings.Builder
	for _, result := range results {
		content, ok, readErr := svc.ReadMemory(ctx, result.Path)
		if readErr != nil {
			return readErr
		}
		if ok {
	...
	answer, err := ai.NewConversation(aiClient, wd).Complete(ctx,
		"Answer only from the supplied persistent memories. State clearly when they do not contain the answer.",
		fmt.Sprintf("Question: %s\n\nRelevant memories:\n%s", query, contextBuilder.String()))
	if err != nil {
		return err
	}
	p.Step("Consulted %d memory record(s)", len(results))
	p.Blank()
	p.Data(answer)
	return nil
}
...
func runMemoryImport(projectID string) error {
	...
	task := p.StartTask("Preparing authoritative memory context %q...", projectID)
	if err := svc.IndexMemories(ctx); err != nil {
		task.Fail("Index preparation failed: %v", err)
		return err
	}
	task.Done("Memory context is queryable directly at %s", svc.TableURI())
...
func runMemoryClean() error {
	p := output.NewPrinter("")
	p.Info("Memory has no derived local store to remove; the authoritative table was left unchanged.")
	return nil
}
...
func runMemoryRemoveContext(contextName string) error {
	...
	if err := ms.PruneScope(contextName, contextName); err != nil {
		return err
	}
	p.Info("Memory context %q disconnected; its local table was removed if present and any remote authoritative table was left unchanged.", contextName)
	wd, _ := os.Getwd()
...
func runMemoryConsolidate(userScope, dryRun bool) error {
	...
	report, err := memory.RunConsolidationInDir(ctx, scope, wd, aiClient)
	if err != nil {
...
func runWikiSearch(query string, wikiRefs, hubRefs []string, sessionName string, continueSession bool, topK int, mode string, aiOptimized bool) error {
	...
	conversation := ai.ResumeConversation(aiClient, wd, session.AgentSessionID, session.AgentCLI)
	result, err := wiki.SearchMultiWiki(ctx, conversation, query, wiki.MultiWikiSearchConfig{
		Sources:           wikiSources,
		WorkDir:           wd,
		UseBM25:           useBM25,
	...
	p.Blank()
	p.Data(result.Answer)
	if err := session.SetAgentSession(result.AgentSessionID, result.AgentCLI); err != nil {
		p.Warn("Failed to save agent session: %v", err)
	}
...
func runWikiChat(sessionID string, continueSession bool) error {
	if nonInteractive {
		return errors.New("wiki chat is interactive and cannot run with --non-interactive")
	}
	p := output.NewPrinter("")
	ctx := context.Background()
...
func openKnowledgeWikiDB(ctx context.Context, projectDir string) (*wiki.WikiDB, error) {
	wikiDir := knowledge.WikiDirForContextIn(projectDir, "")
	if wikiDir == "" {
		return nil, fmt.Errorf("no knowledge wiki for %s — it has not been built yet", projectDir)
	}
	return wiki.OpenWikiDB(ctx, wikiDir)
}

func knowledgeWikiDir(contextName, projectDir string) string {
	return knowledge.WikiDirForContextIn(projectDir, contextName)
}

func runWikiSource(page, contextName, projectDir string, req textslice.Request) error {
	p := output.NewPrinter("")
...
func runWikiBrowse(docType string, limit int, aiOptimized bool) error {
	...
	db, err := openKnowledgeWikiDB(ctx, wd)
	if err != nil {
...
func runWikiLog(limit int, aiOptimized bool) error {
	ctx := context.Background()
	p := output.NewPrinter("")
...
func runWikiXRefs(query string, depth int, aiOptimized bool) error {
	ctx := context.Background()
	p := output.NewPrinter("")
...
func runWikiEmbed() error {
	...
	if len(targets) == 0 {
		return fmt.Errorf("no knowledge wiki to embed")
	}

	p.Running("Generating embeddings for the knowledge wiki…")
