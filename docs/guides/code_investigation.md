...
1. Open **Relationship map**. On first entry, Graphit loads a bounded index sample unless Query lab has already supplied a result.
...
4. Select an entity to open its neighborhood. Incoming entities appear on the left, the selected entity in the center and outgoing entities on the right. Each connection retains its relationship type. On narrow screens these sections stack in reading order.
5. Follow a neighbor to continue exploring; **Back** returns to the preceding entity. **All boundaries** returns to the catalogue. No physics, camera positioning or color interpretation is needed to read direction.
6. Choose **Inspect source & impact** to open the implementation in Find & inspect. There you can read indexed source and explicitly query incoming, outgoing or potential-impact evidence against the selected index.

The boundary search matches names and paths within the loaded result. Language, entity-type and relationship filters narrow that result; they do not fetch additional repository data. **Clear filters** recovers a filtered-empty view. The common header refresh reloads the current result; valid exploration selections remain available. Changing project or imported context resets the exploration.
...
All counts and neighborhood links describe the **loaded result**, not the entire repository. The initial sample includes up to 300 initial nodes and the endpoints of up to 1,000 relationships, so it can contain more than 300 entities. It has no completeness guarantee. Filtering also removes connections to hidden endpoints. No visible link does not prove independence; dynamic dispatch and unresolved calls may be absent even in a wider query.

## Ask a precise question

Use **Find & inspect** for full-index ranked search and **Query lab** for a reproducible read-only Cypher question. Inspect the available schema and persistent identity before traversing. AI drafting produces an editable query; you decide when to run it. Scalar rows remain a table and are never converted into invented graph relationships.
...
- **Request failed:** read the error and use the common header refresh to retry. An error is not evidence of an empty repository.
