...
| Kiro | **No separate hook approval is documented.** | Only when a user disabled a hook, or after sync when a new session has not loaded it yet. | Enable both Graphit entries in **Agent Hooks**, then start a new IDE or CLI session. |
...
4. Review the project hooks from `.codex/hooks.json` and trust the Graphit `SessionStart` and `SubagentStart` hooks.
...
**When.** User action is needed only if either Graphit entry was disabled, or after synchronization when the current IDE or CLI session predates the new hook.

**Action.** If necessary, open **Agent Hooks** and enable `graphit-memory-session-start` and `graphit-memory-session-start-cli` with the eye toggle or **Hook Enabled** switch. Then start a new IDE session or CLI agent.
