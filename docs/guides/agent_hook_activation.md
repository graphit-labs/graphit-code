...
In a fresh main-agent session, ask the agent to summarize the `Graphit session bootstrap` from its current instructions and list its `graphit_*` tools. Then ask it to delegate the same check to a subagent. The first answer verifies hook delivery; the tool list verifies MCP visibility; the delegated answer verifies the child boundary and its tool policy. On the next normal prompt, only the compact Graphit-first invariant should be added; mandatory memory, mandates, rules, and the startup bootstrap must not be reinjected on every turn.
