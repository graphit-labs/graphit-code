...
In a fresh main-agent session, ask the agent to summarize the `Graphit session bootstrap` from its current instructions and list its `graphit_*` tools. Confirm that it independently requires focused non-mandatory Memory search and focused prior/current Task search, followed by `graphit_memory_source` and `graphit_task_get` reads of relevant results. Then ask it to delegate the same check to a subagent. The first answer verifies hook delivery; the tool list verifies MCP visibility; the delegated answer verifies the child boundary and its tool policy. Cursor's `preToolUse(Task)` injection and Antigravity's delegated-prompt compensation must carry the complete protocol when no dedicated subagent context hook exists. On the next normal prompt, only the compact Graphit-first invariant should be added; the searches must not be requested again merely because a recurring boundary ran. Cursor and Antigravity receive only their own compact gap compensation at their documented boundaries, and no other adapter should contain those instructions. Mandatory memory, full mandates, rules, and the startup bootstrap must not be reinjected on every turn.
