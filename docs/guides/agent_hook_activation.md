...
| Deep Code | **No separate hook approval is documented.** | After sync or a change to `.deepcode/AGENTS.md`/the notify helper. | Start Deep Code from the project. An existing non-Graphit `notify` setting is preserved and reported as a conflict. |
...
**Approval.** No separate approval is documented, but Deep Code exposes only the completion
`notify` callback, not a general context-producing hook API.

**When and action.** Run `graphit sync --agent deepcode`, then start `deepcode` from the project.
Graphit maintains a marked block in `.deepcode/AGENTS.md` for bootstrap, routing, checkpoints,
installed Hub rules, and Hub agent instructions. `.deepcode/settings.json` points `notify` to the
absolute path of the project-local `.deepcode/hooks/graphit-notify.sh` or `.cmd`, as required by
Deep Code's native schema; the helper dispatches final sync. If the user
already owns `notify`, sync fails explicitly and leaves that setting unchanged.

**Verify.** Confirm the managed AGENTS block and project MCP entries are present, and that ending a
turn invokes the helper without waiting for indexing. This is instruction compatibility plus a
real final callback; it is not native SessionStart/PostToolUse/SubagentStart enforcement.
