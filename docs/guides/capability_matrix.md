...
|---|---|---|---|---|
| Setup, init, sync, update, remove | Yes | Yes | No | `agent`, `cli`, module switches, installed artifacts |
...
| Knowledge-wiki full-text, semantic, hybrid, browse, xrefs, log, source | Yes | Yes | Knowledge Explorer | Wiki retrieval is deterministic and agent-independent |
| AI wiki answers and chat sessions | Yes | No | Yes | Requires `modules.agent=true` and a supported agent CLI |
| Persistent project and user memory | Yes | Yes | Memory Explorer | One authoritative LanceDB table per scope, direct search/read, optional S3-compatible shared storage |
...
| Agent adapter synchronization and lifecycle hooks | Via init/sync | Via init/sync | No | `agent`, installed rules/skills/agents/commands/MCP/languages, `modules.*` mandates |
...
| Local/direct/broker second-stage reranking | Yes | AST and Knowledge keyword/hybrid search | `search.rerank=true` widens retrieval, preserves candidates, and applies the active provider's reranker after RRF/lexical retrieval. Direct mode supports native Cohere/Voyage/Jina and embedding-simulated OpenAI/OpenAI-compatible/Google; broker configuration requires broker mode for both embedding and rerank. |
...
Hub project discovery is deny-by-default. The effective set is the union of global, direct-user,
and team grants and is returned in cursor-bounded pages. Exact content and remote mounts revalidate
access; `~/.<brand>/hub/cache/` improves repeated reads but has no authorization capability.
