...
Context-capable adapters do not create or update `AGENTS.md`, `CLAUDE.md`, or equivalents to deliver these instructions. Deep Code is the explicit exception: it maintains only its marked compatibility block in `.deepcode/AGENTS.md`, preserving user content. Skills remain physical in native host directories because hosts must discover and load them on demand.
