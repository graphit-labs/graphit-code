...
A delegated performer is not a second coordinator. It receives the session and task ids it needs, so instructing it to open or claim a session only produces a duplicate one — which is what happened while the subagent protocol was a copy of the coordinator bootstrap. The protocol is now scoped by role: it carries the routing invariant, the mandate rules for the modules that role actually uses, and three contracts. The performer claims at most its own task and never creates, claims or closes a coordination session. Returning an answer is not finishing: it reports and stops, leaving the decision that the work is over to the coordinator. And each role states what it must return.
...
Nothing in that protocol claims the performer keeps running between instructions. On every host studied the invocation ends, and continuation comes from resuming it — verified in Claude Code, Cursor and Kimi, and not verified elsewhere. The contract is about posture, not about staying alive.
