...
and prior Dream reports. It never reads, claims, or executes Graphit tasks.
...
### Why reports default to `.graphit/runtime/dream/`

Graphit Task state lives in authoritative LanceDB tables and is independent of Dream — see
[Task Module](task_module.md). The reports vault defaults to runtime storage because of what the
directory holds:
...
A report is generated output, and the other two files are machine state. None is task-control
state.
