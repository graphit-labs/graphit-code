...
- **Source**: The `rules/` prefix of the shared S3-backed Hub selected by the active provider/profile.
...
├── InstallSkill()          → installs the resolved skill into the Agent skills dir
│   └── calls brand.ResolveModuleSkill(module, defaultContent)
└── RemoveSkill()           → removes the skill from the Agent skills dir
```

At hook execution time, `_session-hook` selects only enabled modules and composes their `MandateTrigger()` output. No module install function writes a mandate into an Agent instruction file.
...
Skills are detailed instruction files that agents read on demand from the Agent's skills directory. Module mandates are compact routing summaries delivered by native lifecycle hooks. Separately installed Hub `rule` artifacts are also hook-delivered and are never copied to an Agent rule directory.

### Skill Frontmatter

Every managed `SKILL.md` opens with a YAML frontmatter block carrying exactly two fields, built
by `agent.SkillFrontmatter` (`internal/hub/adapters/agent/adapters.go`) and prepended to the
resolved skill content:

| Field | Contract |
|---|---|
| `name` | must equal the skill's directory name: lowercase letters, digits and single separating hyphens, at most `agent.MaxSkillNameLength` (64) characters |
| `description` | non-empty, at most `agent.MaxSkillDescriptionLength` (1024) characters, counted in runes |
...
frontmatter does not parse is not degraded, it is **invisible**: the Agent discovers no metadata,
...
fails the sync, because every value outside these limits is dropped by the Agent in the same
...
instead of shipping skills no strict Agent will load.

`internal/hub/adapters/agent/frontmatter_test.go` locks the serialization down with values a
...
every supported Agent and reads them back the way an Agent would, and additionally asserts that the
