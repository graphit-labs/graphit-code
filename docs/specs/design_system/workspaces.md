...
| `/ast/explorer`, `/:contextId` | Where is the implementation and what does it affect? Find & inspect → source and typed relations; Query lab and optional map | Scoped search; source navigation/copy; incoming/outgoing/two-hop potential impact; related type selection; editable AI query draft then explicit run; schema/color/language/cluster filters; 2D/3D, physics, fit/zoom and files | `ast/ExplorerPage.tsx`, `QueryBar.tsx`, `CodePanel.tsx`, `GraphCanvas.tsx` |
...
| AST Query lab | Content-first `query-lab-layout` | Query authoring primary, vocabulary alongside, results below |
...
AST map entry loads the bounded sample once per scope when no query has supplied a result. Place collapsible appearance, physics and file controls above the canvas; let the canvas consume remaining viewport space. Query lab renders tabular rows inline, preserving tables returned with graph entities and clear empty/error outcomes.
