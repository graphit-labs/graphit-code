...
2. If available, runs `graphit sync </dev/null >/dev/null 2>&1 &` (backgrounded, detached from stdio).
...
Injects a content block into `.gitignore` using the brand's ignore marker. Used to ensure the `.graphit/` directory is ignored.
