...
## 🔍 Registro Global de Projetos e Descoberta Dinâmica de Clusters
...
graph TD
    ...
    B -->|Consulta Dinâmica via MCP/CLI| D[Projeto A]
    B -->|Consulta Dinâmica via MCP/CLI| E[Projeto B]
```
...
2. **Auto-Descoberta de Irmãos (Sibling Projects):**
   A descoberta de projetos irmãos é feita de forma dinâmica (sob demanda) via ferramenta de CLI (`graphit cluster projects`) ou ferramenta do MCP (`graphit_cluster_projects`). O sistema consulta o mapa global do usuário, encontra todos os outros projetos registrados que compartilham pelo menos um par chave/valor de cluster com o projeto atual e retorna os seus metadados em tempo real.

---

## 👥 Colaboração em Ambientes Corporativos de Microserviços

A existência de projetos vizinhos auto-descobertos dinamicamente permite uma série de superpoderes colaborativos integrados:

### 1. Escrita de Código Guiada por Relações Cruzadas
Quando um desenvolvedor está editando o código do *Projeto A* e precisa chamar um serviço HTTP exposto pelo *Projeto B*, a IA (orquestrada via MCP) chama a ferramenta `graphit_cluster_projects` para saber o caminho físico do *Projeto B* no disco local. Ela então consegue navegar e analisar as rotas, DTOs e assinaturas de métodos do projeto vizinho em tempo real, gerando a chamada do serviço de forma 100% precisa.
...
```
O motor de busca mapeia o identificador `auth-service` para o diretório físico descoberto dinamicamente e realiza o parsing da documentação do serviço irmão de forma totalmente transparente para o usuário.
