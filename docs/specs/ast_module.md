...
**Icebug 0.19 scan limitation.** Native wildcard relationships (`-[r]->`, `-[]-`,
`-->`) and multiple-type relationship patterns (`[:A|B]`) can reuse a previous
physical table's Parquet reader and return incorrect endpoints. The narrow
one-hop `RETURN n,r,m` (or a subset of those variables) proxy described below
enumerates physical forward members before this guard. Other wildcard and
type-alternation forms are rejected before native execution, for ordinary and
paged queries alike. Use the filtered logical traversal forms above or the
Relationship map's bounded sample. The sample queries forward members
...
The selected rerank manifest is resolved only when reranking is turned on. Its fetch policy then
decides whether missing artifacts are downloaded or must already be present. See
`internal/ai/model_catalog.go` and `internal/ai/rerank_local.go`.
...
A related cost that was pure waste: `NewLocalEmbeddingClient` used to provision model artifacts **before**
...
### A published context is mounted by version

A Hub AST context is opened directly from its versioned S3 prefix. The graph is an Icebug bundle
queried by LadybugDB and the search index is a LanceDB directory beside it; installing records the
...
forward members and the manifest records the logical edge total. `/api/contexts`, `/api/schema`,
and exact global count/group queries are answered from those fields before LadybugDB is mounted.
Reverse CSR mirrors are never included in public edge counts. Exact node counts may name one label;
queries with filters, typed relationship patterns, additional projections, or otherwise different
semantics continue through the normal planner or engine instead of being approximated from metadata.

The graph and search logical keys are rooted below
...
Use each type’s declared identity: File/Directory use `path`, while symbols and Module use `uid`. Available properties vary by context; Module must not be assumed to provide source path or line fields. A typed projection such as `RETURN DISTINCT n.uid AS identity, n.name AS name` supports simple aliases. A typed target in bounded traversal restricts final results, while intermediate nodes can have other labels. Every canonical relationship has a nonempty `uid` property derived from its logical type, endpoint identities and indexed occurrence evidence; an ordinal distinguishes otherwise identical occurrences. Unchanged relations keep the same UID through an unrelated incremental reindex. Native `ID(r)` is snapshot metadata and is never a continuation or navigation key. Older bundles without relation UIDs require reindexing; incremental export rebuilds them instead of mixing old and new identity contracts.

### Interactive query results

`GET /api/graph?cypher_query=...` preserves the table alongside extracted graph entities. Node variables may use any alias. A complete returned node contains its `uid` (or File/Directory `path`); a complete returned relation contains `Properties.uid`. Extracted links carry `uid` and native `id` when present. `tabular` always contains `columns` and `rows` arrays for explicit queries, including an empty result; mixed projections retain scalar values. The canonical query proxy safely enumerates physical members for simple one-hop `MATCH (n)-[r]-(m) RETURN n,r,m` and single-variable `RETURN n`, `RETURN r`, or `RETURN m` forms, optionally with typed labels, a UID/path equality anchor and `LIMIT`. This preserves parallel relations and avoids Icebug wildcard scans. Direct unbounded triple results are capped at 1,000 rows and fail explicitly if larger; `QueryPage` reads only the requested prefix, up to 10,000 rows. Use a UID/path anchor or `LIMIT` for larger questions. Other arbitrary Cypher forms retain their existing planner/refusal rules. Read-only clause validation ignores quoted strings, escaped identifiers and comments. Query refusals return HTTP 400 with the engine's explanation; an old relation schema returns a reindex error.
...
The Relationship map loads a bounded index sample on first entry when no explicit search or Cypher request is active. Both entry points select the map while executing and keep results, empty states and errors there; they never trigger a replacement sample. Re-entering preserves the result and local exploration. Its deterministic catalogue groups by directory, file, language or configured cluster; the reader shows directed boundary crossings and entity neighborhoods. Counts deduplicate repeated rows by relation UID, preserving parallel occurrences and discarding edges whose endpoints are unavailable. The sample includes up to 300 initial nodes plus endpoints of up to 1,000 relationships; it is not a complete graph or a runtime impact guarantee. Sample nodes carry indexed `uid`, including pathless external symbols; File/Directory nodes use `path`. Clusters are configured path groups, not inferred communities.

Selecting an entity independently queries all schema-declared incoming/outgoing direct endpoint partitions through `GET /api/graph/neighborhood`, anchored by node UID/path. Each response returns actual nodes, links with relation `uid` and native `id`, `next_cursor` (the last relation UID on a nonfinal page) and `index_generation`. Pages contain up to 100 relations and are ordered by relation UID; no `SKIP` or native ID cursor is used. Each typed member query applies the UID cursor, ordering and page limit in Ladybug before the server merges member pages. The server reads typed forward relation members to avoid Icebug's multi-table endpoint defect. Four requests run concurrently at most. The UI remaps node IDs to declared persistent identities and keys links by relation UID; obsolete selections stop scheduling work. If the generation changes between pages, it restarts all partitions for the same node UID and replaces stale evidence. A second concurrent generation change gives a retryable error. Missing relation UID causes an explicit reindex error. Direct one-hop traversal retains self relationships and parallel edges; multi-hop visited-node pruning remains unchanged. Failed partitions preserve successful evidence and can be retried. Exploration never mutates original result nodes, links or query rows. Source and wider potential-impact queries remain explicit operations. See [Code investigation](../guides/code_investigation.md).
