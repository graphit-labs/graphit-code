...
The Relationship map loads a bounded index sample on first entry when no explicit search or Cypher request is active. Both entry points select the map while executing and keep results, empty states and errors there; they never trigger a replacement sample. Re-entering preserves the result and local exploration. Its deterministic catalogue groups by directory, file, language or configured cluster; the reader shows directed boundary crossings and entity neighborhoods. Counts deduplicate source/type/target triples within the loaded result, discarding edges whose endpoints are unavailable. The sample includes up to 300 initial nodes plus endpoints of up to 1,000 relationships; it is not a complete graph or a runtime impact guarantee. Clusters are configured path groups, not inferred communities. Selecting an entity independently queries all schema-declared incoming/outgoing direct endpoint partitions with its persistent identity. Search-only identities first resolve by label/name/path/line, requiring exactly one indexed entity. Each typed query projects identity plus declared descriptive fields for one reached endpoint, with `ORDER BY identity LIMIT 101`; display 100 and continue with `n.<identity_property> > <last displayed identity>`. No `SKIP` is used. The one-row lookahead is not the continuation cursor. Four requests run concurrently at most; navigation stops scheduling obsolete partitions and discards stale responses. Direct one-hop traversal retains self relationships and edges between selected anchors; multi-hop visited-node pruning remains unchanged. The UI unions forward/reverse observations by persistent source/type/target, so forward-only self relationships appear once. Failed partitions preserve successful evidence and can be retried. Pagination bounds transfer/rendering, not the canonical traversal’s full materialization cost. Exploration never mutates original result nodes, links or query rows. Source and wider potential-impact queries remain explicit operations. See [Code investigation](../guides/code_investigation.md).
