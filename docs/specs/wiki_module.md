...
The Wiki module compiles documentation into a structured, agent-navigable LanceDB store. Page
content, metadata, embeddings, cross-references, and sync history are tables in one `index.lance/`
directory; rendered Markdown is an output format, not a second store. Memory is not a wiki: its
authoritative table is queried directly as described in the Memory Module specification.
...
Memory is deliberately never published as a versioned knowledge artifact. Its read-write,
multi-writer LanceDB table remains authoritative and queryable in place; no machine compiles a
local memory projection. The distinction is mutability and ownership, not the underlying engine.
...
The Knowledge index and the Task store answer different parts of project history. An agent that searches
the project or an imported Knowledge wiki also performs one focused `graphit_task_search` for the same
question when Graphit Task is enabled and available. It follows `next_cursor` until relevant history is
covered and reads every selected task authoritatively with `graphit_task_get`, including its specification,
analytical result, progress, comments, check evidence, next step, and audit history. The Task record
supplements the Knowledge pages read with `graphit_wiki_source`; neither source replaces the other.
Knowledge retrieval remains available when Task is disabled or its tools are unavailable.

This is an agent workflow delivered by the Task mandate and the Knowledge and Task skills, not a backend
join or a change to wiki search results. Full lifecycle-hook context includes the Task mandate whenever
both modules are enabled. Compact recurring hook payloads do not repeat the rule.
