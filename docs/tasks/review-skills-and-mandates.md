...
**`ast_index` via MCP writes to the wrong project's graph.** ~~`openASTDBReadWrite` does a `chdir`, builds `DefaultLadybugConfig()` with a relative `DBPath`, and returns a handle whose database only actually opens on the first query — by which point something else has already changed the working directory back. Fixed~~, along with the `DeleteRepository` stub — see `docs/tasks/fix-indexing-in-wrong-project.md`. The root cause turned out to be more extensive than this paragraph suggested: the same defect showed up in four more places, including an `os.RemoveAll` on a relative path in `ast_index(reset: true)` that deleted the AST database of the wrong project.
