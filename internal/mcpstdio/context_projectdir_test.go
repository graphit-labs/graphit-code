...
func TestResolveWikiDirOnlyCoversKnowledgeScopesAndContexts(t *testing.T) {
	isolateHome(t)
	target := t.TempDir()

	if got := resolveWikiDir("memory", target, "project"); got != "" {
		t.Errorf("memory has no wiki directory, got %q", got)
	}
	if got := resolveWikiDir("memory", target, "some-context"); got != "" {
		t.Errorf("memory context has no wiki directory, got %q", got)
	}
