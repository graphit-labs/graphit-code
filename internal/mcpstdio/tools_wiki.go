...
type wikiSearchInput struct {
	...
	Wikis       []string `json:"wikis,omitempty" jsonschema:"Knowledge wiki sources to search (project or project IDs from ecosystem)"`
	...
	ProjectDir  string   `json:"project_dir,omitempty" jsonschema:"Project directory. Omit only when searching explicit hub_refs."`
...
type wikiBrowseInput struct {
	ProjectDir  string `json:"project_dir,omitempty" jsonschema:"Project directory. Omit to read a globally installed knowledge artifact (context as id@version)."`
...
func searchWikiScope(ctx context.Context, projectDir, scope, mode, query string, vector []float32, topK int) ([]wiki.WikiSearchResult, error) {
	...
	switch mode {
	case "semantic":
		return db.SemanticSearch(ctx, vector, topK)
	case "hybrid":
		return db.HybridSearch(ctx, query, vector, topK)
	default:
		return db.Search(ctx, query, topK)
	}
}
...
func resolveWikiScopeDirContext(projectDir, wikiScope, contextName string) (string, error) {
	...
	default:
		return "", fmt.Errorf("unknown knowledge wiki scope %q — use 'project'", wikiScope)
	}
}
...
func registerWikiTools(server *mcp.Server) {
	mcp.AddTool(server, &mcp.Tool{
		...
		Description: "Search across multiple wiki sources using BM25 full-text and optional semantic search. " +
			...
			"Without project_dir, only explicit hub_refs are searchable, because every wiki source names a project.",
		...
		wikis := input.Wikis
		if len(wikis) == 0 && projectDir != "" {
			wikis = []string{"project"}
		}

		if projectDir == "" && len(wikis) > 0 {
			return errResult(fmt.Errorf("wiki sources need a project: without project_dir only explicit hub_refs are searchable"))
		}
		...
		default:
			...
			for _, scope := range wikis {
				...
				results, err := searchWikiScope(ctx, projectDir, scope, searchMode, input.Query, queryVec, topK)
				if err != nil {
					skipped = append(skipped, err.Error())
					continue
				}
	...
	mcp.AddTool(server, &mcp.Tool{
		...
		projectDir, err := resolveWikiScope(input.ProjectDir, input.Context)
		if err != nil {
			return errResult(err)
		}
		...
		db, err := openWikiForReadContext(ctx, projectDir, "knowledge", input.Context)
		if err != nil {
			return errResult(err)
		}
		defer func() { _ = db.Close() }()
	...
	mcp.AddTool(server, &mcp.Tool{
		...
		if len(targets) == 0 {
			return errResult(fmt.Errorf("no knowledge wiki to embed"))
		}
	...
	mcp.AddTool(server, &mcp.Tool{
		...
		wikiDir := resolveWikiDir("knowledge", projectDir, contextName)
