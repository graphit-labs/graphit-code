...
// A knowledge wiki needs either a project or a named installed artifact.
func TestResolveWikiScopeRequiresKnowledgeOwner(t *testing.T) {
	cases := []struct {
		name      string
		context   string
		wantError bool
	}{
		{"knowledge needs a context", "", true},
		{"knowledge with a context is fine", "demo-kb@1.0.0", false},
	}
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			_, err := resolveWikiScope("", c.context)
			if (err != nil) != c.wantError {
				t.Errorf("resolveWikiScope(\"\", %q) error = %v, wantError = %v",
					c.context, err, c.wantError)
			}
		})
	}
}
...
func TestTheGlobalScopeReadsNoProjectConfiguration(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(bystander, brand.LockFileName()),
		[]byte(`{"project":{"id":"01BYSTANDER"},"config":{"hub.bucket":"someone-elses-bucket"},"agents":["claude"]}`),
		0o600); err != nil {
	...
	if cfg, agents := loadProjectLockInfo(""); cfg != nil || agents != nil {
		t.Errorf("loadProjectLockInfo(\"\") = (%v, %v), want (nil, nil)", cfg, agents)
	}
