...
func resolveWikiScope(projectDir, contextName string) (string, error) {
	abs, err := resolveProjectDirOptional(projectDir)
	if err != nil {
		return "", err
	}
...
func resolveAgentFromProject(agent, projectDir string) string {
	projectCfg, agents := loadProjectLockInfo(projectDir)
	return config.ResolveProjectAgent(agent, nil, projectCfg, agents)
}
