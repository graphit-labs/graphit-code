...
func TestTextResult(t *testing.T) {
	...
	want := "hello world" + agent.SysReminder
	if tc.Text != want {
...
func TestAgentInputsSerializeOnlyAgentField(t *testing.T) {
	inputs := []any{
		initInput{Agent: "codex"},
		syncInput{Agent: "codex"},
		updateInput{Agent: "codex"},
		removeInput{Agent: "codex"},
		hubInstallInput{Agent: "codex"},
		hubUninstallInput{Agent: "codex"},
		hubUpdateInput{Agent: "codex"},
		hubTypePathInput{Agent: "codex"},
	}
	...
	for _, input := range inputs {
		...
		body := string(encoded)
		if !strings.Contains(body, `"agent":"codex"`) {
			t.Errorf("%T does not expose the agent field: %s", input, body)
		}
		if strings.Contains(body, `"ide"`) {
			t.Errorf("%T still exposes the removed ide field: %s", input, body)
		}
	}

	var decoded initInput
	if err := json.Unmarshal([]byte(`{"ide":"cursor"}`), &decoded); err != nil {
		t.Fatal(err)
	}
	if decoded.Agent != "" {
		t.Fatalf("removed ide field unexpectedly populated Agent: %q", decoded.Agent)
	}
}
