...
func exportDirectWithReverse(ri *rebuildIndex, outDir, storageURI string, filterLabels, filterRels map[string]bool, reverse bool) (*ladybug.CanonicalManifest, error) {
	...
	writeCollectedNodeTable := func(c collectedNodeTable) nodeWriteResult {
		...
		res.row = ladybug.CanonicalNodeTable{
			...
			LangCounts: canonicalLangCountsDirect(c.table.stringCounts("lang")),
		}
		return res
	}
...
func canonicalLangCountsDirect(counts map[string]int64) []ladybug.CanonicalLangCount {
	langs := make([]string, 0, len(counts))
	for lang := range counts {
		langs = append(langs, lang)
	}
	sort.Strings(langs)
	out := make([]ladybug.CanonicalLangCount, 0, len(langs))
	for _, lang := range langs {
		out = append(out, ladybug.CanonicalLangCount{Lang: lang, Rows: counts[lang]})
	}
	return out
}
