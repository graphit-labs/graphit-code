...
func ASTRuleContent() string {
	return strings.Join([]string{
		...
		"| --- | --- |",
		"| Search code: Grep, rg, grep, search_text, workspace search | Unknown file: `" + brand.MCPToolName("ast_search") + "`, then verify selected hits with `" + brand.MCPToolName("ast_source") + "`. Known file: source with pattern and context. |",
		"| Locate a file: Glob, find, file search | `" + brand.MCPToolName("ast_schema") + "` once, then `" + brand.MCPToolName("ast_query") + "` on File.relative_path with a prefix/suffix/substring filter. This queries indexed files, not arbitrary filesystem globs. |",
		"| Read code: Read, read_file, cat, open_file | `" + brand.MCPToolName("ast_source") + "` with a returned relative path; select an entity or line range instead of dumping a file. |",
		...
		"",
		"Unknown location — `" + brand.MCPToolName("ast_search") + "`:",
		"```json",
		"{\"project_dir\":\"/work/app\",\"query\":\"ParseOrder\",\"mode\":\"fts\",\"top_k\":5,\"ai_optimized\":true}",
		"```",
		"",
		"Known-file grep with surrounding evidence — `" + brand.MCPToolName("ast_source") + "`:",
		"```json",
		...
		"",
		"Before writing an impact assessment, reconciling code with a documented contract, or handing an investigation to another agent, read [the completed impact case](references/impact-assessment.md). It shows observations, limits, test selection and a reusable result. Read it only for that workflow; simple lookups use the recipes above. If the local resource is unavailable, call `" + brand.MCPToolName("module_skill") + "` with `module: ast`, `reference: references/impact-assessment.md` and the known `project_dir` when available.",
		"",
		...
		"",
		"For a database-open failure retry once, then inspect `" + brand.MCPToolName("daemon_status") + "` and report the limitation. For a schema rejection refresh the schema once and correct the query; an unsupported planner form calls for a simpler supported query, not repeated guessing. For an absent current-project graph use `" + brand.MCPToolName("ast_index") + "`, scoped by `path` when possible. Use `" + brand.MCPToolName("ast_embed") + "` only for missing semantic coverage; FTS may still work. Do not reset a store to repair a read.",
		"",
		"Use native discovery only when the required MCP tool is unavailable or current-project text is unsupported/unindexed; state this fallback once and do not substitute the Graphit CLI. The daemon indexes writes; use `" + brand.MCPToolName("sync") + "` only when a decision requires proven freshness. The adapter stop hook handles completion sync asynchronously; do not duplicate or wait for it. Context installation/export/removal are maintenance operations only when required; removal without `context` clears the project graph.",
	}, "\n") + "\n"
}
