...
// SkillReferences keeps complete cases out of routine source-lookup context.
func SkillReferences() map[string]string {
	return map[string]string{
		"references/impact-assessment.md": strings.NewReplacer(
			"TOOL_SCHEMA", brand.MCPToolName("ast_schema"),
			"TOOL_QUERY", brand.MCPToolName("ast_query"),
			"TOOL_SOURCE", brand.MCPToolName("ast_source"),
		).Replace(astImpactAssessment),
	}
}

const astImpactAssessment = `# Complete impact assessment
...
Call TOOL_SCHEMA with {"project_dir":"/work/graphit-code"} once. The observed schema supplies Function, uid, path, line_number, end_line, cyclomatic_complexity and CALLS.
...
Observed identity: internal/ast/rule_compact.go::ASTRuleContent; stored cyclomatic complexity: 1. Read it with TOOL_SOURCE:
...
For the first row, call TOOL_SOURCE with path internal/ast/rule.go and entity InstallSkill. For the MCP row, a focused source pattern ASTRuleContent with before/after context locates the dispatch; expand to the handler's resolution/override logic before concluding parity.
