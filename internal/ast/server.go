...
func (s *Server) handleSchema(w http.ResponseWriter, r *http.Request) {
	...
	nodeStats, err := schemaNodeStats(ctx, db)
	if err != nil {
		writeError(w, http.StatusInternalServerError, err.Error())
		return
	}
...
func schemaNodeStats(ctx context.Context, db GraphDB) ([]map[string]any, error) {
	if provider, ok := db.(canonicalStatsProvider); ok {
		if stats, canonical := provider.canonicalGraphStats(); canonical {
			out := make([]map[string]any, 0, len(stats.Nodes))
			for _, stat := range stats.Nodes {
				out = append(out, map[string]any{"label": stat.Label, "count": stat.Count})
			}
			return out, nil
		}
	}
	...
	out := make([]map[string]any, 0, len(labelsRes.Records))
	for _, record := range labelsRes.Records {
		out = append(out, map[string]any{"label": record["label"], "count": record["count"]})
	}
	return out, nil
}
...
func schemaEdgeStats(ctx context.Context, db GraphDB) []map[string]any {
	if provider, ok := db.(canonicalStatsProvider); ok {
		if stats, canonical := provider.canonicalGraphStats(); canonical {
			out := make([]map[string]any, 0, len(stats.Relationships))
			for _, stat := range stats.Relationships {
...
func (s *Server) handleContexts(w http.ResponseWriter, r *http.Request) {
	...
	if _, err := os.Stat(projectDir); err == nil {
		var projectDB GraphDB
		if !isDifferentProject {
			projectDB = s.db
		} else {
			projectDB = s.getOrCreateCachedDB(targetDir, projectDir, true)
		}
		nodeCount, edgeCount := graphCounts(ctx, projectDB)
		backend := "ladybug"
		if !isDifferentProject {
			backend = s.db.BackendType()
		}
	...
	for key, ictx := range ListImportedContextsIn(targetDir) {
		...
		ic["node_count"], ic["edge_count"] = graphCounts(ctx, ctxDB)
...
func graphCounts(ctx context.Context, db GraphDB) (int, int) {
	if provider, ok := db.(canonicalStatsProvider); ok {
		if stats, canonical := provider.canonicalGraphStats(); canonical {
			return int(stats.NodeCount), int(stats.EdgeCount)
		}
	}

	nodeCount, edgeCount := 0, 0
	if res, err := db.Query(ctx, "MATCH (n) RETURN count(n) AS c", nil); err == nil && len(res.Records) > 0 {
		nodeCount = toInt(res.Records[0]["c"])
	}
	if res, err := db.Query(ctx, "MATCH ()-[r]->() RETURN count(r) AS c", nil); err == nil && len(res.Records) > 0 {
		edgeCount = toInt(res.Records[0]["c"])
	}
...
func (s *Server) handleGenerateCypher(w http.ResponseWriter, r *http.Request) {
	...
	repoPath := generationRepoPath(body.ProjectDir, s.repoPath)
	...
	result := map[string]any{
		...
		"agent_session_id": resp.AgentSessionID,
		"agent_cli":        resp.AgentCLI,
	}
	if resp.Error != "" {
...
func generationRepoPath(requestedProjectDir, serverRepoPath string) string {
	if requestedProjectDir != "" {
		return requestedProjectDir
	}
	return serverRepoPath
}
