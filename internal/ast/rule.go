...
func ASTRuleContent() string {
	lines := []string{
		...
		"**conceptual/intent-based** rather than name-based — call the " + brand.MCPToolRef("ast", "search_semantic") + " tool (passing absolute `project_dir` and natural language `query`):",
		"```",
		"" + brand.MCPToolName("ast", "search_semantic") + "(project_dir: \"/path/to/project\", query: \"authentication and session management\")",
		"```",
		"",
		"> ⚠️ **CRITICAL: " + brand.MCPToolRef("ast", "search_semantic") + " accepts PLAIN TEXT only — NEVER Cypher.**",
		...
		"> ✅ Correct: Call " + brand.MCPToolRef("ast", "search_semantic") + " with `query: \"payment processing\"`",
		"> ❌ Wrong: Call " + brand.MCPToolRef("ast", "search_semantic") + " with `query: \"MATCH (f:Function) WHERE ...\"`",
		"",
		...
		"If results are empty, call the " + brand.MCPToolRef("sync") + " tool (passing `project_dir`) to start background sync and generate embeddings for future use.",
		"",
		...
		"or source code patterns** — call the " + brand.MCPToolRef("ast", "search_fts") + " tool (passing absolute `project_dir` and keywords `query`):",
		"```",
		"" + brand.MCPToolName("ast", "search_fts") + "(project_dir: \"/path/to/project\", query: \"processOrder\")",
		"```",
		"",
		"> ⚠️ **CRITICAL: " + brand.MCPToolRef("ast", "search_fts") + " accepts PLAIN TEXT only — NEVER Cypher.**",
		...
		"> ✅ Correct: Call " + brand.MCPToolRef("ast", "search_fts") + " with `query: \"payment\"`",
		"> ❌ Wrong: Call " + brand.MCPToolRef("ast", "search_fts") + " with `query: \"MATCH (f:File) WHERE ...\"`",
		"",
		...
		"comments, or code fragments — without using grep: call " + brand.MCPToolRef("ast", "search_fts") + " (passing absolute `project_dir` and keyword `query`):",
		"```",
		"# Search for a string literal or error message across all source files",
		"" + brand.MCPToolName("ast", "search_fts") + "(project_dir: \"/path/to/project\", query: \"connection refused\")",
		"",
		"# Find files containing a specific code pattern",
		"" + brand.MCPToolName("ast", "search_fts") + "(project_dir: \"/path/to/project\", query: \"TODO refactor\")",
		"```",
		"",
		"**FTS vs Semantic:** Use " + brand.MCPToolRef("ast", "search_fts") + " for exact keyword matching, " + brand.MCPToolRef("ast", "search_semantic") + " for",
...
func ASTRouterContent() string {
	lines := []string{
		...
		"|---|---|",
		"| `grep_search: func myFunction` | " + brand.MCPToolRef("ast", "query") + " with `query: \"MATCH (f:Function) WHERE toLower(f.name) CONTAINS 'myfunction' RETURN f.name, f.path, f.line_number\"`, `ai_optimized: true` |",
		"| `grep_search: type MyStruct` | " + brand.MCPToolRef("ast", "query") + " with `query: \"MATCH (n) WHERE toLower(n.name) CONTAINS 'mystruct' RETURN n.name, label(n) AS type, n.path\"`, `ai_optimized: true` |",
		...
		"| `grep -l \"keyword\" *.go` | " + brand.MCPToolRef("ast", "search_fts") + " with `query: \"keyword\"` |",
		"| `find ... -name \"*.go\" \\| xargs grep -l \"daemon\"` | " + brand.MCPToolRef("ast", "search_fts") + " with `query: \"daemon\"` |",
		"| Searching for an entity that could be more than one type | " + brand.MCPToolRef("ast", "query") + " with `query: \"MATCH (f) WHERE (label(f) = 'Function' OR label(f) = 'Method') AND toLower(f.name) CONTAINS 'search' RETURN f.name, f.path, f.line_number, label(f) AS type\"`, `ai_optimized: true` |",
		"",
		"## Quick Reference (always active)",
		"",
		"- **Always use**: call " + brand.MCPToolRef("ast", "query") + " tool (passing absolute `project_dir` and setting `ai_optimized: true`)",
		"- **Discover node labels**: call " + brand.MCPToolRef("ast", "schema") + " tool (passing absolute `project_dir`)",
		"- **Never guess names**: Ground with `toLower(n.name) CONTAINS toLower('keyword')`",
		"- **Semantic search**: call " + brand.MCPToolRef("ast", "search_semantic") + " (passing absolute `project_dir` and natural language `query`)",
		"- **FTS (source text search)**: call " + brand.MCPToolRef("ast", "search_fts") + " (passing absolute `project_dir` and keyword `query`). Searches entity names AND `:File` source content. Use instead of grep.",
		"- **Get source code (discovery)**: call " + brand.MCPToolRef("ast", "source") + " (passing absolute `project_dir` and relative `path`). Retrieves source from the graph when you discovered a file through AST. Supports `head`/`tail` (first/last N lines), `start_line`/`end_line` (line range), `entity`/`entity_type` (extract entity source by name), `pattern`/`regex`/`before`/`after` (grep-like search with context), and `line_numbers`. If you already know the path, use your IDE's file-reading tools instead.",
		"- **One-shot: get metadata + full file source**: call " + brand.MCPToolRef("ast", "query") + " with `query: \"MATCH (fn:Function {name: 'Validate'})<-[:CONTAINS]-(file:File) RETURN fn.name, fn.line_number, fn.end_line, file.path, file.source\"`, `ai_optimized: true`",
		"- **Reindex after changes**: call " + brand.MCPToolRef("sync") + " tool (passing absolute `project_dir`)",
		"",
		...
		"8. **" + brand.MCPToolRef("ast", "search_fts") + " and " + brand.MCPToolRef("ast", "search_semantic") + " = PLAIN TEXT only** — these parameters accept keywords or natural language, NEVER Cypher queries. Only " + brand.MCPToolRef("ast", "query") + " accepts Cypher.",
		...
		"   - Find function: call " + brand.MCPToolRef("ast", "query") + " with `query: \\\"MATCH (f:Function) WHERE toLower(f.name) CONTAINS 'NAME' RETURN f.name, f.path, f.line_number\\\"`, `ai_optimized: true`",
		"   - Find callers: call " + brand.MCPToolRef("ast", "query") + " with `query: \\\"MATCH (a)-[:CALLS]->(b {name: 'NAME'}) RETURN a.name, a.path\\\"`, `ai_optimized: true`",
		"   - Full-text search: call " + brand.MCPToolRef("ast", "search_fts") + " with `query: \\\"KEYWORD\\\"`",
