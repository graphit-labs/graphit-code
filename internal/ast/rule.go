...
func ASTRuleContent() string {
	lines := []string{
		...
		"",
		"Call the " + brand.MCPToolRef("ast", "source") + " tool (passing absolute `project_dir` and `path`):",
		"```",
		...
		"",
		"Use a **two-step pattern**: first call " + brand.MCPToolRef("ast", "query") + " to find the entity's location, then extract its source using the file-reading tools or " + brand.MCPToolRef("ast", "source") + ".",
		"",
		"#### 4c. Get source of all functions in a file with their line ranges",
		"",
		"1. Call " + brand.MCPToolRef("ast", "query") + " with `query: \"MATCH (file:File {path: 'src/services/payment.go'})-[:CONTAINS]->(fn:Function) RETURN fn.name, fn.line_number, fn.end_line ORDER BY fn.line_number\"` to get line ranges.",
		"2. Call " + brand.MCPToolRef("ast", "source") + " with `path: \"src/services/payment.go\"` to get the source content.",
		"",
		"#### 4d. Quick source peek — verify a function's implementation",
		"",
		"One-shot query via " + brand.MCPToolRef("ast", "query") + ":",
		"```",
...
func ASTRouterContent() string {
	lines := []string{
		...
		"- **Get source code (discovery)**: call " + brand.MCPToolRef("ast", "source") + " (passing absolute `project_dir` and relative `path`). Retrieves source from the graph when you discovered a file through AST. If you already know the path, use your IDE's file-reading tools instead.",
