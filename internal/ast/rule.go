...
func ASTRuleContent() string {
	lines := []string{
		...
		"| Semantic search for \"who calls X\" | Call " + brand.MCPToolRef("ast", "query") + " with `query: \"MATCH (a)-[:CALLS]->(b:Function {name: 'X'}) RETURN DISTINCT a.name, a.path\"` | AST: exact CALLS edges. Semantic: guesses from text proximity |",
		...
		"| Reading files to understand structure | Call " + brand.MCPToolRef("ast", "query") + " with `query: \"MATCH (f:File {path: 'X'})-[:CONTAINS]->(e:Function) RETURN DISTINCT e.name, e.line_number ORDER BY e.line_number\"` | AST: instant file skeleton. File reading: manual, token-heavy |",
		"| `grep` for import usage | Call " + brand.MCPToolRef("ast", "query") + " with `query: \"MATCH (f:File)-[:IMPORTS]->(m:Module {name: 'X'}) RETURN DISTINCT f.path\"` | AST: pre-resolved import graph. Grep: regex on `import` statements |",
		"| Searching for class hierarchy | Call " + brand.MCPToolRef("ast", "query") + " with `query: \"MATCH (a:Class)-[:INHERITS*]->(b:Class {name: 'X'}) RETURN DISTINCT a.name\"` | AST: transitive closure in one query. Manual: file-by-file tracing |",
		"",
		...
		"to fall back to grep: " + brand.MCPToolRef("knowledge", "search") + ", " +
			brand.MCPToolRef("wiki", "browse") + ", " + brand.MCPToolRef("wiki", "source") + ".",
		...
		"that is plainly in the repository, read those two keys with " + brand.MCPToolRef("config", "get") + " before",
		...
		"T-SQL repository has no Procedure and no Package in its graph, read `ast.grammar` with " + brand.MCPToolRef("config", "get") + "",
		...
		"`OPTIONAL MATCH`. " + brand.MCPToolRef("ast", "query") + " runs arbitrary Cypher against it.",
		"",
		"**This is the most powerful tool you have here, and the one most often left unused.** The",
		"failure mode is specific and worth naming: an agent runs " + brand.MCPToolRef("ast", "search") + ",",
		...
		"",
		"| The question | " + brand.MCPToolRef("ast", "search") + " gives you | A Cypher query gives you |",
		"|---|---|---|",
		...
		"",
		"🔒 **MANDATORY: the first AST call you make is " + brand.MCPToolRef("ast", "schema") + ", not a query.**",
		...
		"| `n.body`, `n.code`, `n.text`, `n.content` | source text is not on entity nodes | " + brand.MCPToolRef("ast", "source") + " |",
		...
		"| `n.returns`, `n.return_type` | not indexed | read the declaration with " + brand.MCPToolRef("ast", "source") + " |",
		...
		"" + brand.MCPToolRef("ast", "schema") + " and read that label's row, then rewrite once.** A guessing",
		...
		"   expect thirty is a rebuild, not a typo. Retry before rewriting anything; " +
			brand.MCPToolRef("ast", "schema") + " and " + brand.MCPToolRef("daemon", "status") + " settle it.",
		"",
		...
		"> is actually queryable. Source code comes from " + brand.MCPToolRef("ast", "source") + ", never from a `RETURN f.source`.",
		"",
		...
		"using Reciprocal Rank Fusion (RRF, k=60) into one ranking — call the " + brand.MCPToolRef("ast", "search") + " tool (passing absolute `project_dir` and `query`):",
		"```",
		...
		"> clear — pair this query with a hybrid search (" + brand.MCPToolRef("ast", "search") + ") on the",
		...
		"> Call " + brand.MCPToolRef("ast", "schema") + " first when you are about to rely on either.",
		"",
		...
		"the location; " + brand.MCPToolRef("ast", "source") + " gives you the text at it:",
		"```",
		"MATCH (fn:Function {name: 'Validate'})<-[:CONTAINS]-(file:File) RETURN file.path, fn.line_number, fn.end_line",
		"```",
		"then pass that `path` — with `entity` or the line range — to " + brand.MCPToolRef("ast", "source") + ".",
		"",
		...
		"- Call " + brand.MCPToolRef("ast", "list") + " first — the system may already be an imported context; querying an existing one beats re-searching the Hub",
		"- Otherwise call " + brand.MCPToolRef("hub", "search") + " with the system's name and `type: \"knowledge\"` (and `type: \"ast\"` if you need its source, not just its docs) to find pre-built artifacts",
		...
		"",
		"# Step 4b: Call chain",
		"" + brand.MCPToolName("ast", "query") + "(project_dir: \"/path/to/project\", query: \"MATCH (a:Function {name: 'RunSync'})-[:CALLS]->(b) RETURN DISTINCT b.name, b.path\")",
		"",
		...
		"## Cypher Guidelines",
		"",
		"- 🔒 **Schema before Cypher.** Call " + brand.MCPToolRef("ast", "schema") + " before the first query of a session, and again after switching `project_dir` or `context`. Guessing a property name does not return an empty result — it crashes with `Binder exception: Cannot find property`.",
		...
		"| **a Hub artifact, and you have no project at all** | " + brand.MCPToolRef("hub", "install") + " with NO `project_dir`, then pass the qualified `id@version` as `context` — also with no `project_dir` |",
		"",
		"### Querying with no project — the qualified identifier replaces it",
		"",
		"**`project_dir` is optional on the read tools** — " + brand.MCPToolRef("ast", "schema") + ", " +
			brand.MCPToolRef("ast", "query") + ", " + brand.MCPToolRef("ast", "search") + ", " +
			brand.MCPToolRef("ast", "source") + ", " + brand.MCPToolRef("ast", "list") +
			" — for the case where there is no checkout on this machine to name: an agent reaching this",
		...
		"" + brand.MCPToolName("ast", "list") + "()",
		"" + brand.MCPToolName("ast", "schema") + "(context: \"<artifact-id>@<version>\")",
		"" + brand.MCPToolName("ast", "query") + "(context: \"<artifact-id>@<version>\", query: \"MATCH (f:Function) RETURN f.name, f.path\")",
		"" + brand.MCPToolName("ast", "source") + "(context: \"<artifact-id>@<version>\", path: \"<path>\", entity: \"<name>\")",
		"```",
		"",
		...
		"- **It is read-only.** " + brand.MCPToolRef("ast", "index") + " and " + brand.MCPToolRef("ast", "embed") +
			" need a project, because opening a store read-write CREATES it and there",
		"  is no identity to key one by.",
		"- **`" + brand.MCPToolName("ast", "list") + "()` with no `project_dir` is how you find out what is reachable** — it lists the",
		...
		"a fraction of the work. " + brand.MCPToolRef("sync") + " is for when you genuinely want every",
		"subsystem rebuilt at once — which is exactly once per session, at the end, below.",
		"",
		"### 🔒 MANDATORY: one " + brand.MCPToolRef("sync") + " when you finish a change session",
		"",
		"**Before you report a session that changed code as done, call " + brand.MCPToolRef("sync"),
		...
		"| once, when the change session is finished | " + brand.MCPToolRef("sync") + " | it makes every index's currency a fact you established rather than one you assumed |",
		"",
		"**Why sync and not " + brand.MCPToolRef("ast", "index") + ", when this is the AST skill:** because a",
		"session that changed code changed more than code. You wrote a task log, and most likely a",
		"memory or two, and each of those lives in an index of its own. " + brand.MCPToolRef("ast", "index"),
		...
		"by looking at it.",
		"",
		"So the trigger for calling " + brand.MCPToolRef("sync") + " mid-session is not \"I edited\" — it is",
		...
		"When it is only the graph you doubt, " + brand.MCPToolRef("ast", "index") + " with a `path` gets there",
		"faster; " + brand.MCPToolRef("sync") + " is what puts the graph, the documentation wiki and the",
		"memory wikis at the same point, which is what \"the indexes are current\" actually means.",
		"",
		"So: **the last tool call of a session that touched code is " + brand.MCPToolRef("sync") + ".**",
		"Reach for " + brand.MCPToolRef("ast", "index") + " instead when the code graph alone is the",
		...
		"",
		"1. **A graph is never a file you read.** " + brand.MCPToolRef("ast", "source") + " is how source",
		...
		"   is why " + brand.MCPToolRef("ast", "list") + " needs a `project_dir` and answers differently",
...
func MandateTrigger() string {
	return ide.ModuleMandateTrigger(
		...
		[]string{
			...
			"you are finishing a session in which you changed code — one " + brand.MCPToolName("sync") + " before you report it done, so the next session opens a current graph, wiki and memory instead of assuming three",
			"you are about to open a graph, a store or a shard as a file — you cannot: every store is global and keyed by id, so source text comes from " + brand.MCPToolName("ast", "source") + " and nothing else",
		},
