...
func ASTRuleContent() string {
	lines := []string{
		...
		"`cyclomatic_complexity` and `entry_point_score` are properties on every callable; you do not",
		"count branches by hand and you do not guess which functions are entry points.",
		"```bash",
		...
		"MATCH ()-[:CALLS]->(s:Function) WITH collect(DISTINCT s.name) AS called MATCH (f:Function) WHERE f.is_stub = false AND NOT f.name IN called AND f.cyclomatic_complexity > 10 AND f.entry_point_score < 10 RETURN f.name, f.cyclomatic_complexity, f.path ORDER BY f.cyclomatic_complexity DESC",
		"",
		"# where the system is entered from the outside",
		...
		"",
		"# the doors in: handlers, mains, tests",
		"MATCH (f:Function) WHERE f.entry_point_score > 50 RETURN f.name, f.path ORDER BY f.entry_point_score DESC",
		"",
		...
		"",
		"#### 🔒 Every callable exists TWICE: the declaration and the call target",
		"",
		"This is the single most important fact about this graph, and every query that mixes calls with",
		"locations depends on it.",
		"",
		"| | `is_stub: false` — the **declaration** | `is_stub: true` — the **call target** |",
		"|---|---|---|",
		"| Created by | the file that declares it | a call site referring to it by name |",
		"| `path`, `line_number` | the real ones | **empty and `0`** |",
		"| `uid` | `internal/x/y.go::Apply.Apply` | just `Apply` |",
		"| Reached by | `CONTAINS` from its `File` | `CALLS` from its callers |",
		"",
		"Real example from this repository, one name, two nodes:",
		"```",
		"MATCH (t:Function {name: 'Apply'}) RETURN t.name, t.path, t.line_number, t.is_stub",
		"→ false | 53 | Apply | internal/textslice/textslice.go",
		"→ true  |  0 | Apply |",
		"```",
		"",
		"**Three consequences you must design queries around:**",
		"",
		"1. **`CALLS` never points at a declaration.** So `MATCH (f:Function) WHERE NOT ()-[:CALLS]->(f)`",
		"   is true for *every* declaration — it does **not** find dead code, it finds everything. Same",
		"   for `OPTIONAL MATCH (caller)-[:CALLS]->(f)` counting `0` on a function with fifty callers.",
		"   Comparing callers to declarations must go **through the name**, never through node identity.",
		"2. **You cannot chain `CALLS` and `CONTAINS` through one variable.** A pattern like",
		"   `(caller)-[:CALLS]->(e)<-[:CONTAINS]-(f:File)` returns **zero rows and no error**, because no",
		"   single node has both edges. Two clauses joined by a shared variable across the two edge types",
		"   fail the same way. This silence is the trap: an empty result reads like \"nothing matches\".",
		"3. **A call target has no location.** Filtering a `CALLS` target by `path` or `line_number`",
		"   always returns nothing. To find *where* something is, match the declaration; to find *who",
		"   calls it*, match by name.",
		"",
		"**The workaround, and it is a good one:** `WITH collect(...)` works, so you can gather one side",
		"and filter the other against it in a single query.",
		"```bash",
		"# genuinely uncalled functions — collect the called NAMES, then exclude declarations by name",
		"MATCH ()-[:CALLS]->(s:Function) WITH collect(DISTINCT s.name) AS called MATCH (f:Function) WHERE f.is_stub = false AND NOT f.name IN called AND f.entry_point_score < 10 RETURN f.name, f.path, f.cyclomatic_complexity ORDER BY f.cyclomatic_complexity DESC",
		"",
		"# how many callers a specific function really has",
		"MATCH (caller)-[:CALLS]->(t:Function {name: 'Apply'}) RETURN count(caller) AS callers",
		"```",
		"",
		"> Add `f.is_stub = false` to any query that returns `path` or `line_number`, or you will report",
		"> results with an empty path and line `0` as if they were real locations.",
		"",
		...
		"MATCH ()-[:CALLS]->(s:Function) WITH collect(DISTINCT s.name) AS called MATCH (f:Function) WHERE f.is_stub = false AND NOT f.name IN called AND f.entry_point_score < 10 RETURN f.name, f.path",
		"",
