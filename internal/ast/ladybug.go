...
func (k *LadybugBackend) prepareRemoteAccessLocked() error {
...
func (k *LadybugBackend) refreshRemoteAccessLocked() error {
	...
	for _, statement := range ladybug.S3ConfigStatements(resolvedLadybugS3Credentials(refreshed)) {
		if err := k.execQueryLocked(statement); err != nil {
			return fmt.Errorf("apply renewed scoped S3 access for Icebug: %w", err)
		}
	}
...
func (k *LadybugBackend) loadCanonicalManifestLocked() error {
	...
	for _, p := range candidates {
		...
		if man.Format != "icebug-canonical" || !man.Finished {
			return fmt.Errorf("icebug: unsupported manifest format %q (icebug-canonical required)", man.Format)
		}
		k.canonical = &man
		return nil
	}
...
func (k *LadybugBackend) openOnce() error {
	...
	if err := k.prepareRemoteAccessLocked(); err != nil {
		k.conn.Close()
		k.db.Close()
		k.conn, k.db = nil, nil
		return err
...
func (k *LadybugBackend) ensureConnected() error {
	...
	if k.conn != nil {
		return k.refreshRemoteAccessLocked()
	}
...
func (k *LadybugBackend) Query(ctx context.Context, cypher string, params map[string]any) (*QueryResult, error) {
	...
	if k.canonical == nil {
		if err := k.loadCanonicalManifestLocked(); err != nil {
			return nil, fmt.Errorf("icebug: canonical manifest: %w", err)
		}
	}
	if k.canonical != nil {
		if result, handled := canonicalMetadataQuery(k.canonical, cypher); handled {
			return result, nil
		}
	}
...
func (k *LadybugBackend) QueryPage(ctx context.Context, cypher string, params map[string]any, offset, limit int) (*QueryResult, error) {
	...
	if k.canonical != nil {
		if result, handled := canonicalMetadataQuery(k.canonical, cypher); handled {
			return sliceQueryResult(result, offset, limit), nil
		}
	}
	if err := k.ensureConnected(); err != nil {
		return nil, err
