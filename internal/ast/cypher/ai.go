...
import (
	...
	agentai "github.com/graphit-labs/graphit-code/internal/ai"
...
func (g *Generator) Generate(ctx context.Context, req QueryRequest) (*QueryResponse, error) {
	...
	conversation := agentai.NewConversation(g.ai, req.RepoPath)

	keywords := []string{req.UserQuery}
	var entities []string
	if !ast.IsCanonicalGraph(g.db) {
		var err error
		keywords, err = g.expandKeywordsWithClient(ctx, conversation, req.UserQuery)
		if err != nil {
	...
	cypher, err := g.generateCypherWithClient(ctx, conversation, req.UserQuery, schema, keywords, entities, req.RepoPath, req.MaxResults)
	if err != nil {
		return nil, fmt.Errorf("cypher generation: %w", err)
	}

	cypher = sanitizeQuery(cypher)
	cypher = ast.PublicCypher(g.db, cypher)
...
func (g *Generator) expandKeywords(ctx context.Context, query, repoPath string) ([]string, error) {
	return g.expandKeywordsWithClient(ctx, agentai.NewConversation(g.ai, repoPath), query)
}

func (g *Generator) expandKeywordsWithClient(ctx context.Context, client AIClient, query string) ([]string, error) {
	response, err := client.Complete(ctx, keywordSystemPrompt, query)
	if err != nil {
		return nil, err
	}
...
func (g *Generator) generateCypher(
	...
	return g.generateCypherWithClient(ctx, agentai.NewConversation(g.ai, repoPath), userQuery, schema, keywords, entities, repoPath, maxResults)
}

func (g *Generator) generateCypherWithClient(
	ctx context.Context,
	client AIClient,
	userQuery, schema string,
	keywords, entities []string,
