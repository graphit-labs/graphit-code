...
func (m *mockGraphDB) IsCanonicalGraph() bool {
	return m.canonical
}
...
func (m *mockAIClient) CompleteInDir(ctx context.Context, workingDir, systemPrompt, userPrompt string) (string, error) {
...
func TestGenerateWithQueryError(t *testing.T) {
	...
	gen := NewGenerator(db, ai)
	resp, err := gen.Generate(context.Background(), QueryRequest{UserQuery: "test", MaxResults: 5, Execute: true})
	if err != nil {
...
func TestCanonicalGenerateSkipsKeywordExpansionAndGlobalPreSearch(t *testing.T) {
	var aiCalls int
	var globalSearches int
	var generatedExecutions int
	db := &mockGraphDB{
		canonical: true,
		queryFunc: func(ctx context.Context, cypher string, params map[string]any) (*ast.QueryResult, error) {
			if strings.Contains(cypher, "toLower(n.name) CONTAINS") {
				globalSearches++
			}
			if cypher == "MATCH (n:Directory)-[r:contains]-(m:File) RETURN n, r, m" {
				generatedExecutions++
			}
			return &ast.QueryResult{}, nil
		},
	}
	ai := &mockAIClient{completeFunc: func(ctx context.Context, systemPrompt, userPrompt string) (string, error) {
		aiCalls++
		return "<cypher>MATCH (n:Directory)-[r:contains]-(m:File) RETURN n, r, m</cypher>", nil
	}}

	resp, err := NewGenerator(db, ai).Generate(context.Background(), QueryRequest{
		UserQuery: "o que tem relação com init.d?",
		RepoPath:  "/projects/linux",
	})
	if err != nil {
		t.Fatalf("Generate: %v", err)
	}
	if aiCalls != 1 {
		t.Fatalf("AI calls = %d, want one generation call without keyword expansion", aiCalls)
	}
	if globalSearches != 0 {
		t.Fatalf("canonical generation ran %d global keyword searches", globalSearches)
	}
	if generatedExecutions != 0 {
		t.Fatalf("generation executed the generated query %d times", generatedExecutions)
	}
	if len(resp.Keywords) != 1 || resp.Keywords[0] != "o que tem relação com init.d?" {
		t.Fatalf("keywords = %v, want the original question as local grounding", resp.Keywords)
	}
	if ai.workingDir != "/projects/linux" {
		t.Fatalf("agent working directory = %q, want requested project", ai.workingDir)
	}
}

func TestCanonicalGenerateUsesLogicalManifestSchemaAndNormalizesModelOutput(t *testing.T) {
	icebugDir := filepath.Join(t.TempDir(), "graph.icebug")
	if err := os.MkdirAll(icebugDir, 0o755); err != nil {
		t.Fatalf("mkdir: %v", err)
	}
	manifest := &ladybug.CanonicalManifest{
		Version:  ladybug.CanonicalManifestVersion,
		Format:   "icebug-canonical",
		Finished: true,
		NodeTables: []ladybug.CanonicalNodeTable{
			{Label: "Directory", Rows: 1},
			{Label: "File", Rows: 1},
		},
		RelGroups: []ladybug.CanonicalRelGroup{{
			Type: "contains",
			Members: []ladybug.CanonicalMember{{
				From: "Directory", To: "File", Table: "contains__directory_file", Rows: 1,
			}},
			ReverseMembers: []ladybug.CanonicalMember{{
				From: "File", To: "Directory", Table: "contains__directory_file_reverse", Rows: 1,
			}},
		}},
	}
	...
	db := ast.NewLadybugDBReadOnly(ast.LadybugConfig{IcebugDir: icebugDir})
	ai := &mockAIClient{completeFunc: func(ctx context.Context, systemPrompt, userPrompt string) (string, error) {
		if !strings.Contains(systemPrompt, "(Directory)-[:contains]->(File)") {
			t.Fatalf("logical manifest relationship missing from prompt:\n%s", systemPrompt)
		}
		if strings.Contains(systemPrompt, "contains__directory_file") {
			t.Fatalf("physical relationship leaked into prompt:\n%s", systemPrompt)
		}
		return "<cypher>MATCH (n:Directory)-[r:contains__directory_file]-(m:File) WHERE toLower(n.name) CONTAINS toLower('init.d') RETURN n, r, m</cypher>", nil
	}}
	...
	want := "MATCH (n:Directory)-[r:contains]-(m:File) WHERE toLower(n.name) CONTAINS toLower('init.d') RETURN n, r, m"
	if resp.Cypher != want {
		t.Fatalf("Cypher = %q, want %q", resp.Cypher, want)
	}
}

func TestGenerateStopsAfterCanceledKeywordExpansion(t *testing.T) {
	ctx, cancel := context.WithCancel(context.Background())
	var dbQueries int
	db := &mockGraphDB{queryFunc: func(ctx context.Context, cypher string, params map[string]any) (*ast.QueryResult, error) {
		dbQueries++
		return &ast.QueryResult{}, nil
	}}
	ai := &mockAIClient{completeFunc: func(callCtx context.Context, systemPrompt, userPrompt string) (string, error) {
		cancel()
		return "", context.Canceled
	}}

	_, err := NewGenerator(db, ai).Generate(ctx, QueryRequest{UserQuery: "find payment"})
	if !errors.Is(err, context.Canceled) {
		t.Fatalf("Generate error = %v, want context.Canceled", err)
	}
	if dbQueries != 0 {
		t.Fatalf("canceled generation continued with %d DB queries", dbQueries)
	}
}

func TestGenerateStopsAfterCanceledSchema(t *testing.T) {
	ctx, cancel := context.WithCancel(context.Background())
	var aiCalls int
	db := &mockGraphDB{queryFunc: func(ctx context.Context, cypher string, params map[string]any) (*ast.QueryResult, error) {
	...
	_, err := NewGenerator(db, ai).Generate(ctx, QueryRequest{UserQuery: "find init.d"})
	if !errors.Is(err, context.Canceled) {
		t.Fatalf("Generate error = %v, want context.Canceled", err)
	}
	if aiCalls != 1 {
		t.Fatalf("AI calls = %d, want keyword expansion only", aiCalls)
	}
}
