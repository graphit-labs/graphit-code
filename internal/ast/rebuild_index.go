...
func (ri *rebuildIndex) stubTableJSON() []map[string]any {
	...
	for _, fe := range ri.fileEntries {
		for _, ref := range fe.entry.References {
			if ref.SourceUID != "" && !ri.emittedAny(ref.TargetUID) {
				ri.emitUID(ref.TargetUID, "Table")
				rows = append(rows, stubJSON(ref.TargetUID))
			}
		}
	}
	return rows
...
func (ri *rebuildIndex) dmlEdgeJSON(relType, srcLabel string) []map[string]any {
	var rows []map[string]any
	for _, fe := range ri.fileEntries {
		for _, ref := range fe.entry.References {
			if ref.RelType == relType && ref.SourceUID != "" && ri.entityUIDs[ref.SourceUID] == srcLabel &&
				ri.emittedAny(ref.SourceUID) && ri.emittedAny(ref.TargetUID) {
				rows = append(rows, map[string]any{
					"source_uid": ref.SourceUID, "target_uid": ref.TargetUID,
