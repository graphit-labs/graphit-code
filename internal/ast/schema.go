...
func SchemaLangGroups(ctx context.Context, db GraphDB) []SchemaLangGroup {
	if provider, ok := db.(canonicalStatsProvider); ok {
		if stats, canonical := provider.canonicalGraphStats(); canonical {
			...
			// Old development bundles have no language histogram. Do not turn a missing
			// optimization hint into a full graph scan; a fresh init materializes it.
...
func SchemaText(ctx context.Context, db GraphDB) (string, error) {
	if provider, ok := db.(canonicalManifestProvider); ok {
		if man, available := provider.canonicalManifestSnapshot(); available {
			return canonicalSchemaText(man), nil
		}
	}
...
func canonicalSchemaText(man *ladybug.CanonicalManifest) string {
	stats := canonicalStatsFromManifest(man)
	var buf strings.Builder
	writeSchemaLangSection(&buf, stats.Langs)
	buf.WriteString("Node labels and key properties:\n")

	for _, table := range man.NodeTables {
		props := make([]string, 0, len(table.Columns))
		for _, column := range table.Columns {
			props = append(props, column.Name)
		}
		fmt.Fprintf(&buf, "- %s(%s)\n", table.Label, strings.Join(props, ", "))
	}

	buf.WriteString("\nRelationships (logical public types from the manifest):\n")
	seen := make(map[string]bool)
	for _, group := range man.RelGroups {
		for _, member := range group.Members {
			line := fmt.Sprintf("- (%s)-[:%s]->(%s)\n", member.From, group.Type, member.To)
			if !seen[line] {
	...
	buf.WriteString("\nOnly the logical relationship types above are valid in public Cypher. Physical member tables and reverse mirrors are internal storage details and must never be used.\n")
	return buf.String()
}
