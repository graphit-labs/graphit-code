...
func initAntlrEngine() (*wasmantlr.Engine, error) {
	antlrEngineOnce.Do(func() {
		...
		if antlrEngineErr != nil {
			slog.Error("failed to initialize Wasmtime ANTLR engine", "error", antlrEngineErr)
		}
	})
	return antlrEngine, antlrEngineErr
}

func getAntlrLanguage(grammarName string, projectDir string) (*wasmantlr.Module, func(), error) {
	poolVal, _ := grammarPools.LoadOrStore(grammarName, &sync.Pool{})
	pool := poolVal.(*sync.Pool)

	if val := pool.Get(); val != nil {
		mod := val.(*wasmantlr.Module)
		cleanup := func() {
			pool.Put(mod)
		}
	...
	mod, err := instantiateAntlrLanguage(grammarName, projectDir)
	if err != nil {
		return nil, nil, err
	}
...
func instantiateAntlrLanguage(grammarName string, projectDir string) (*wasmantlr.Module, error) {
	...
	if !hasCompiled {
		...
		if !hasCompiled {
			...
			errLoad := engine.LoadModule(grammarName, wasmBytes)
			if errLoad != nil {
				pluginMu.Unlock()
				return nil, fmt.Errorf("load ANTLR module for %q: %w", grammarName, errLoad)
			}
		}
		pluginMu.Unlock()
	}

	mod, err = engine.InstantiateModule(grammarName)
	if err != nil {
		return nil, fmt.Errorf("instantiate ANTLR module for %q: %w", grammarName, err)
	}
