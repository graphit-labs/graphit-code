...
func buildDreamPrompt(projectDir, sessionID, agent string, outcomes []*memory.ConsolidationOutcome) string {
	context := buildDreamContext(projectDir, sessionID, agent)
...
func buildDreamContext(projectDir, sessionID, agent string) string {
	var b strings.Builder
	...
	b.WriteString("You are NOT making code changes. You are generating and improving **Agent artifacts** ")
	b.WriteString("(skills, rules, commands, memories) that make future conversations more effective.\n\n")

	_, _ = fmt.Fprintf(&b, "You are operating with Agent context `%s`. ", agent)
	b.WriteString("Use this when interacting with the hub, syncing rules, or any Agent-scoped operations. ")
	_, _ = fmt.Fprintf(&b, "For example: `%s sync --agent %s` or `%s hub install --agent %s`.\n\n", brand.BinName(), agent, brand.BinName(), agent)
	...
	_, _ = fmt.Fprintf(&b, "2. List ALL project and user memories with %s (`scope: \"project\"` and `scope: \"user\"`) and read them with %s\n",
		brand.MCPToolRef("memory", "list"), brand.MCPToolRef("memory", "source"))
	b.WriteString("3. Query the AST graph to understand code structure, public APIs, and module boundaries\n")
	b.WriteString("4. Scan installed Agent artifacts — list ALL existing skills, rules, and commands in the project\n")
...
func buildDreamEnvelope(projectDir, sessionID string) string {
	...
	b.WriteString("Document the evaluation of existing Agent artifacts:\n\n")
