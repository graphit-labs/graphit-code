...
// the key upper-cased, dots turned into underscores. `knowledge.docs_dir` becomes
// GRAPHIT_KNOWLEDGE_DOCS_DIR. Authentication is not part of this namespace.
//
...
// FallbackAgent and FallbackCLI are the last resort of every Agent/CLI resolution: what the framework
...
// resolution paths — ResolveAgent, resolveAmbientAgent, ResolveCLI, the unified UI server, and
// CLIForAgent's pairing. As three separate string literals they could be changed one at a time, and
...
//
// The two are kept consistent by CLIForAgent: FallbackAgent's paired CLI IS FallbackCLI, and
// TestFallbackAgentAndCLIAgree fails if that stops being true.
const (
...
func ResolveAgent(flagValue string, inlineCfg, projectCfg ConfigMap) string {
	...
	if val := ResolveConfig("agent", inlineCfg, projectCfg); val != "" {
		return val
	}
...
func ResolveProjectAgent(flagValue string, inlineCfg, projectCfg ConfigMap, lockfileAgents []string) string {
	...
	if inlineCfg != nil {
		if val, ok := GetConfigValue(inlineCfg, "agent"); ok && val != "" {
			return val
		}
	}

	if projectCfg != nil {
		if val, ok := GetConfigValue(projectCfg, "agent"); ok && val != "" {
			return val
		}
	}

	resolved := resolveAmbientAgent()

	if len(lockfileAgents) > 0 {
		for _, registered := range lockfileAgents {
			if strings.EqualFold(registered, resolved) {
				return resolved
			}
		}

		return lockfileAgents[0]
	}

	return resolved
}

func resolveAmbientAgent() string {
	envKey := brand.EnvPrefix() + "_AGENT"
	if val := os.Getenv(envKey); val != "" {
		return val
	}

	if val, ok, _ := GetGlobalConfigValue("agent"); ok && val != "" {
		return val
	}

	defs := getCompiledDefaults()
	if val, ok := GetConfigValue(defs, "agent"); ok && val != "" {
		return val
	}
...
func ResolveCLI(flagValue string, inlineCfg, projectCfg ConfigMap, resolvedAgent string) string {
	...
	if resolvedAgent != "" {
		if cli := CLIForAgent(resolvedAgent); cli != "" {
			return cli
		}
	}
...
func DefaultCLI() string {
	return ResolveCLI("", nil, nil, DefaultAgent())
}

func CLIForAgent(agent string) string {
