...
func newCohereEmbeddingClient(cfg cohereEmbeddingConfig) (EmbeddingClient, error) {
	if strings.TrimSpace(cfg.apiKey) == "" {
		return nil, fmt.Errorf("embedding provider \"cohere\" needs an API key in the active login profile")
	}
	...
	if dim == 0 {
		return nil, fmt.Errorf("cannot determine the embedding vector width for cohere model %q — set --embedding-dimensions on the provider", cfg.model)
	}
