...
func TestOpenAIEmbeddingClient_BrokerSendsQueryAndDocumentInputTypes(t *testing.T) {
	bodies := make(chan map[string]any, 2)
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		var body map[string]any
		_ = json.NewDecoder(r.Body).Decode(&body)
		bodies <- body
		_, _ = w.Write([]byte(`{"data":[{"embedding":[0.1,0.2],"index":0}],"graphit":{"revision":"r1","dimensions":2}}`))
	}))
	defer srv.Close()

	client, err := newOpenAIEmbeddingClient(openAIEmbeddingConfig{
		provider: "broker", endpoint: srv.URL, model: "r1", dimensions: 2, expectedRevision: "r1",
	})
	if err != nil {
		t.Fatal(err)
	}
	queryClient, ok := client.(QueryEmbedder)
	if !ok {
		t.Fatal("broker client does not implement QueryEmbedder")
	}
	if _, err := queryClient.EmbedQuery(context.Background(), "query"); err != nil {
		t.Fatal(err)
	}
	if _, err := client.EmbedBatch(context.Background(), []string{"document"}); err != nil {
		t.Fatal(err)
	}
	for _, inputType := range []string{"query", "document"} {
		body := <-bodies
		if body["input_type"] != inputType {
			t.Fatalf("%s input_type=%v", inputType, body["input_type"])
		}
		if _, ok := body["model"]; ok {
			t.Fatalf("broker request contains model: %v", body)
		}
	}
}
