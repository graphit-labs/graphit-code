...
func TestLazyEmbeddingClient_ModelName_WithClient(t *testing.T) {
	t.Parallel()
	lazy := NewLazyEmbeddingClient()
	lazy.client = &localEmbeddingClient{modelName: "custom-embedder"}
	name := lazy.ModelName()
	if name != "custom-embedder" {
		t.Errorf("ModelName = %q; want %q", name, "custom-embedder")
	}
}
...
func TestFindORTLibrary_PrefersPackagedVersionedName(t *testing.T) {
	if runtime.GOOS == "windows" {
		t.Skip("Windows runtime name is intentionally unversioned")
	}
	dir := t.TempDir()
	versioned, generic := "libonnxruntime.so.1.29.0", "libonnxruntime.so"
	if runtime.GOOS == "darwin" {
		versioned, generic = "libonnxruntime.1.29.0.dylib", "libonnxruntime.dylib"
	}
	for _, name := range []string{generic, versioned} {
		if err := os.WriteFile(filepath.Join(dir, name), []byte("fake"), 0o644); err != nil {
			t.Fatal(err)
		}
	}
	if runtime.GOOS == "darwin" {
		t.Setenv("DYLD_LIBRARY_PATH", dir)
	} else {
		t.Setenv("LD_LIBRARY_PATH", dir)
	}
	if got, want := findORTLibrary(), filepath.Join(dir, versioned); got != want {
		t.Fatalf("findORTLibrary() = %q, want %q", got, want)
	}
}
