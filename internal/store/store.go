...
// ASTHubIcebugDir is the legacy/local cache location for a Hub Icebug bundle. Native Hub mounts use S3 directly.
...
func MemoryTableRoot() string { return globalOr("", "memory") }

// MemoryTableDir is where a scope's authoritative local Lance table lives. It is not a raw
// Markdown store, and broker object storage is deliberately not used for mutable native tables.
...
func memoryScopeSegment(scope, scopeID string) string {
	return fmt.Sprintf("memory-%s-%s", SanitizeSegment(scope), SanitizeSegment(scopeID))
}
