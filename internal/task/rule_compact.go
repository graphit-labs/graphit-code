...
func RuleContent() string {
	...
	return strings.Join([]string{
		...
		"Before creating or materially revising a multi-outcome specification/plan, read [references/planning.md](references/planning.md). Before saving a multi-task backlog or batch, read [references/worked-feature.md](references/worked-feature.md): it fills specification, plan, every leaf, contracts, fixtures, coverage and claim sequence. Reuse these references while retained; do not load them for unrelated reads.",
...
func MandateTrigger() string {
	return agent.ModuleMandateTrigger(
		...
		"Use a known task ID or focused search; read selected records/results. Search prior analysis, decisions and evidence whenever new questions arise during work; reuse sufficient context. Create/claim executable work. Investigate current code/docs and resolve material questions before defining the delivery map. Before implementation, save specification -> plan -> dependency-ordered tasks with acceptance criteria and validations; split multiple deliverables into executable subtasks. Each leaf must be usable by an agent without this conversation. Prefer batch creation for planned packets with known input IDs; inspect each result. Verify requirement coverage and readiness before coding. Keep official state/results in Task, checkpoint meaningful outcomes, and complete only on check evidence. Read other skills when their evidence is needed; avoid repeating already-sufficient searches.",
		nil,
		[]string{"task_get", "task_search", "task_create", "task_batch", "task_claim"},
	)
}
