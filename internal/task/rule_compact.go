...
func RuleContent() string {
	...
	return strings.Join([]string{
		"# Graphit Task",
		"",
		"Graphit Task is the only task-control source of truth. Planning may use other tools, including the host agent's native planning mechanism. Always record and maintain the resulting tasks through Graphit Task tools. The shared LanceDB tables own official task state; external plans, native TODOs and Markdown files cannot replace the Graphit backlog or lifecycle records.",
		...
		"",
		"Use Task tools for every record, regardless of where planning happened. Incorporate externally defined requirements, decisions, specifications, tests, subtasks, interdependencies and milestones into self-contained Graphit tasks; a link to an external plan alone is insufficient. Reconcile subsequent planning changes through the same Task workflow and verify the saved records. Keep the current planning/refinement task separate from future delivery tasks so planning can finish without marking implementation complete. Once registration is authorized, create or revise tasks as definitions settle; preserve every user requirement, correction, decision, rationale and unresolved question. Do not invent missing decisions. Map each requirement to its task IDs and acceptance/test checks in the planning specification; retain this coverage map across splits and scope changes.",
		"",
...
func MandateTrigger() string {
	return agent.ModuleMandateTrigger(
		...
		"Planning may use other tools, including native planning; the resulting task definitions and official state must be recorded through Graphit Task tools. When users plan features or whole systems, offer to register and maintain their backlog through Task tools. At the end, ask about desired work outside the session whose registration is undecided; honor prior authorization or refusal. Preserve all requirements, tests, interdependencies and milestones for another agent to implement correctly and completely. Before material project work, including work that only analyzes and changes no files, use Graphit Task for official work records so ownership, validation, knowledge capture, and resumability are established in LanceDB. Whenever Knowledge is searched, also search related prior/current Graphit tasks with `graphit_task_search`, follow `next_cursor` until relevant history is covered, and read every relevant result with `graphit_task_get`; use the complete Task record alongside Knowledge evidence. Task descriptions must be exhaustive execution-ready reports containing every relevant detail already discovered, so another agent can act without repeating code investigation. Record the complete, detailed analytical result in the task for reuse by later analyses and system learning; apply this same no-loss standard to progress, comments, evidence, reasons, handoffs, and completion summaries.",
		[]string{
			"planning features, systems, milestones, or desired work beyond the current session",
