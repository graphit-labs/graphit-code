...
func MandateTrigger() string {
	return agent.ModuleMandateTrigger(
		"Task", skillName,
		"starting, resuming, planning, analyzing, delegating or completing project work",
		"Use a known task ID or one focused task search; read selected records and prerequisite results, then create/claim the work. Investigate current code/docs and resolve material questions before defining the delivery map. Before implementation, save specification -> plan -> dependency-ordered tasks with acceptance criteria and validations; split multiple deliverables into executable subtasks. Each leaf must be usable by an agent without this conversation. Verify requirement coverage and readiness before coding. Keep official state/results in Task, checkpoint meaningful outcomes, and complete only on check evidence. Read other module skills at their evidence-gathering boundary; reuse context and avoid repeated history searches.",
		nil,
