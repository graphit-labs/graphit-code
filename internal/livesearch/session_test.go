...
func TestCreateReachesReadyAndPersistsMetadata(t *testing.T) {
	...
	s, err := m.Create(Options{Agent: "claude", Title: "why is it slow"})
	if err != nil {
	...
	if meta.State != StateReady || meta.Agent != "claude" || meta.Title != "why is it slow" {
...
func TestListReportsInterruptedSessionsAsFailed(t *testing.T) {
	...
	meta := Meta{ID: id, State: StateRunning, Agent: "claude", CreatedAt: time.Now().UTC()}
	...
	live, err := m.Create(Options{Agent: "claude"})
	if err != nil {
		t.Fatalf("Create: %v", err)
	}
...
func TestCloseAllStopsEverySession(t *testing.T) {
	m := newTestManager(t, echoClient(), nil)
	for i := 0; i < 3; i++ {
		if _, err := m.Create(Options{Agent: "claude"}); err != nil {
			t.Fatalf("Create: %v", err)
		}
	}
	m.CloseAll()
