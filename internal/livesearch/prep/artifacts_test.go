...
func (f *fakeInstaller) Install(_ context.Context, entryID, _, agent string, entryType hub.ArtifactType, _, projectDir string) (*hub.InstallResult, error) {
	call := installCall{entryID: entryID, agent: agent, artType: entryType, dir: projectDir}
...
func prepareWith(t *testing.T, agent string, artifacts []livesearch.Artifact) (*livesearch.Session, []string, error) {
	t.Helper()
	mgr := livesearch.NewManager(t.TempDir(), nil, nil)
	t.Cleanup(mgr.CloseAll)
	s, err := mgr.Create(livesearch.Options{Agent: agent, Artifacts: artifacts})
	if err != nil {
...
func TestArtifactsOfAnyTypeAreInstalledIntoTheWorkspace(t *testing.T) {
	...
	for i, call := range fake.calls {
		...
		if call.agent != "claude" {
			t.Fatalf("artifact %d was installed for agent %q, want claude", i, call.agent)
		}
	}
	var sawPinned bool
