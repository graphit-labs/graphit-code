...
func TestResolveWikiSource_Memory(t *testing.T) {
	tmp := t.TempDir()
	svc := NewWikiService(tmp)
	if _, err := svc.ResolveWikiSource("memory"); err == nil {
		t.Fatal("memory must not resolve as a wiki source")
	}
}
...
func TestResolveSources_ValidWikis(t *testing.T) {
	...
	if len(errs) != 1 {
		t.Errorf("expected the memory-as-wiki rejection, got: %v", errs)
	}
	if len(sources) != 1 {
		t.Errorf("expected only the project wiki source, got %d", len(sources))
	}
}
