...
func TestGetPathsFallbackToCwd(t *testing.T) {
	p := GetPaths("test-agent", false)
	if p == nil {
		t.Fatal("expected non-nil paths")
	}
	if p.Agent != "test-agent" {
		t.Errorf("expected Agent 'test-agent', got %q", p.Agent)
	}
}

func TestGetPathsGlobal(t *testing.T) {
	p := GetPaths("test-agent", true)
	if p == nil {
		t.Fatal("expected non-nil paths")
	}
...
func TestGetPathsForProjectEmpty(t *testing.T) {
	p := GetPathsForProject("test-agent", "")
	if p == nil {
		t.Fatal("expected non-nil paths")
	}
}

func TestBuildPathsDefaultAgent(t *testing.T) {
	p := buildPaths("", "/tmp/project")
	if p.Agent != "antigravity" {
		t.Errorf("expected default Agent 'antigravity', got %q", p.Agent)
	}
}
