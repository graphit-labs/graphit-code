...
func GetPaths(agent string, global bool) *ProjectPaths {
	projectDir, _ := gitTopLevel()
	if projectDir == "" {
		projectDir, _ = os.Getwd()
	}

	p := buildPaths(agent, projectDir)
...
func GetPathsForProject(agent, projectDir string) *ProjectPaths {
	if projectDir == "" {
		return GetPaths(agent, false)
	}
	return buildPaths(agent, projectDir)
}

func buildPaths(agent, projectDir string) *ProjectPaths {
	if agent == "" {
		agent = "antigravity"
	}
