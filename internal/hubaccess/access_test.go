...
func TestTrustedSubjectWrapsAuthenticationStateErrors(t *testing.T) {
	globalDir := t.TempDir()
	t.Setenv(brand.EnvVar("GLOBAL_DIR"), globalDir)
	if err := os.WriteFile(filepath.Join(globalDir, "auth.json"), []byte("{"), 0o600); err != nil {
		t.Fatal(err)
	}

	_, err := TrustedSubject(context.Background())
	if !errors.Is(err, ErrUnauthenticated) {
		t.Fatalf("TrustedSubject error = %v, want ErrUnauthenticated", err)
	}
	if !errors.Is(err, io.ErrUnexpectedEOF) {
		t.Fatalf("TrustedSubject error = %v, want wrapped unexpected EOF", err)
	}
}
...
func TestTrustedSubjectUsesActiveAccountProfile(t *testing.T) {
	...
	if err := store.Login(auth.Profile{Name: "alice", Provider: "local", Username: "alice", Teams: []string{"platform", "security", "platform"}}); err != nil {
		t.Fatal(err)
	}
	subject, err := TrustedSubject(context.Background())
	if err != nil {
