...
func TrustedSubject(ctx context.Context) (Subject, error) {
	...
	if strings.TrimSpace(snapshot.Profile.Username) == "" {
		return Subject{UserID: AnonymousUserID}, nil
	}
	return normalizeSubject(Subject{UserID: snapshot.Profile.Username, TeamIDs: snapshot.Profile.Teams})
}
...
// FromSelectors validates and canonicalizes selectors supplied by another trusted authorization
// backend, such as graphit-hub-access-v1. It never reads projects.json.
func FromSelectors(selectors []Selector) (Grants, error) {
	grants := Grants{ids: make(map[string]struct{})}
	prefixes := make(map[string]struct{})
	for i, selector := range selectors {
		normalized, err := normalizeSelector(selector)
		if err != nil {
			return Grants{}, fmt.Errorf("selector %d: %w", i, err)
		}
		if normalized.All {
			grants.all = true
		}
		if normalized.ID != "" {
			grants.ids[normalized.ID] = struct{}{}
		}
		if normalized.NamePrefix != "" {
			prefixes[normalized.NamePrefix] = struct{}{}
		}
	}
