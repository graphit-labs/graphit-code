...
type sessionAwareAIClient struct {
	workDir   string
	sessionID string
}

func (c *sessionAwareAIClient) Complete(context.Context, string, string) (string, error) {
	return "stateless", nil
}

func (c *sessionAwareAIClient) CompleteWithSession(ctx context.Context, sessionID, systemPrompt, userPrompt string) (string, string, error) {
	return c.CompleteWithSessionInDir(ctx, "", sessionID, systemPrompt, userPrompt)
}

func (c *sessionAwareAIClient) CompleteWithSessionInDir(_ context.Context, workDir, sessionID, _, _ string) (string, string, error) {
	c.workDir = workDir
	c.sessionID = sessionID
	return "continued", "native-next", nil
}

func (c *sessionAwareAIClient) SupportsSession() bool { return true }
func (c *sessionAwareAIClient) AgentCLI() string      { return "codex" }
...
func TestChatEngineResumesAndPersistsNativeAgentSession(t *testing.T) {
	_ = setupChatTestHome(t)
	projectDir := t.TempDir()
	session := NewSession(projectDir, nil, "initial")
	if err := session.SetAgentSession("native-before", "codex"); err != nil {
		t.Fatal(err)
	}
	client := &sessionAwareAIClient{}

	response, err := NewChatEngine(client, session).Send(context.Background(), "follow up")
	if err != nil {
		t.Fatal(err)
	}
	...
	if client.sessionID != "native-before" {
		t.Fatalf("resumed session = %q, want native-before", client.sessionID)
	}
	if client.workDir != projectDir {
		t.Fatalf("work dir = %q, want %q", client.workDir, projectDir)
	}
	...
	if loaded.AgentSessionID != "native-next" || loaded.AgentCLI != "codex" {
		t.Fatalf("persisted agent session = %q/%q", loaded.AgentCLI, loaded.AgentSessionID)
	}
}
