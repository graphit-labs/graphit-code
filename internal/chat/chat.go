...
func NewChatEngine(client AIClient, session *ChatSession) *ChatEngine {
	return &ChatEngine{
		base:    client,
		client:  agentai.ResumeConversation(client, session.ProjectDir, session.AgentSessionID, session.AgentCLI),
		session: session,
	}
}
...
func (e *ChatEngine) Send(ctx context.Context, userMessage string) (string, error) {
	...
	if err := e.session.SetAgentSession(e.client.SessionID(), e.client.AgentCLI()); err != nil {
		return "", fmt.Errorf("saving agent session: %w", err)
	}
...
func (e *ChatEngine) SendWithSearch(ctx context.Context, userMessage string, searchResult *wiki.SearchResult) (string, error) {
	if searchResult != nil && searchResult.AgentSessionID != "" {
		if err := e.session.SetAgentSession(searchResult.AgentSessionID, searchResult.AgentCLI); err != nil {
			return "", fmt.Errorf("saving search agent session: %w", err)
		}
		e.client = agentai.ResumeConversation(e.base, e.session.ProjectDir, searchResult.AgentSessionID, searchResult.AgentCLI)
	}
