...
func TestSessionManagerRefreshesAndPublishesBrokerOIDCProfile(t *testing.T) {
	...
	broker = httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "/token":
			...
			writeStandardBrokerToken(t, w, key, map[string]any{"iss": broker.URL, "sub": "gb_sub_1", "aud": "graphit-cli",
				"iat": now.Unix(), "exp": now.Add(time.Hour).Unix(), "preferred_username": "alice", "organization": "acme", "groups": []string{"platform"}},
				signBrokerJWT(t, key, brokerAccessClaimsForTest(broker.URL, now, "access-2")), "refresh-2")
		default:
			http.NotFound(w, r)
		}
	}))
	...
	if snapshot.Profile.OIDC.AccessToken == "" || snapshot.Profile.OIDC.AccessToken == "access-1" || snapshot.Profile.OIDC.RefreshToken != "refresh-2" {
	...
	persisted, err := store.Active()
	if err != nil || persisted.Profile.OIDC.AccessToken != snapshot.Profile.OIDC.AccessToken || persisted.Profile.OIDC.RefreshToken != "refresh-2" {
...
func TestSessionManagerRefreshesBrokerOIDCWithoutEnablingS3(t *testing.T) {
	...
	broker = httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "/token":
			...
			writeStandardBrokerToken(t, w, key, map[string]any{"iss": broker.URL, "sub": "gb_sub_1", "aud": "graphit-cli",
				"iat": now.Unix(), "exp": now.Add(time.Hour).Unix(), "preferred_username": "alice"},
				signBrokerJWT(t, key, brokerAccessClaimsForTest(broker.URL, now, "access-2")), "refresh-2")
		default:
			http.NotFound(w, r)
		}
