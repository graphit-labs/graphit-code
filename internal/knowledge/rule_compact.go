...
func KnowledgeRuleContent(contexts []string, docsDir string) string {
	...
	return strings.Join([]string{
		...
		"2. When Graphit Task is enabled and its MCP tools are available, pair every Knowledge search with one focused `" + brand.MCPToolName("task", "search") + "` for the same question, pass `ai_optimized: true`, and follow `next_cursor` until the relevant history is covered. Read every relevant hit with `" + brand.MCPToolName("task", "get") + "` and use its specification, analytical result, progress, comments, check evidence, next step, and audit history. Task history supplements Knowledge and never replaces authoritative page reads. If Task is disabled or its tools are unavailable, continue the Knowledge workflow.",
		"3. Pick the smallest relevant page set and read it with `" + brand.MCPToolName("wiki", "source") + "`; use a pattern or line slice for long pages. Use `preview: true` only when titles cannot disambiguate.",
		"4. Use `" + brand.MCPToolName("wiki", "xrefs") + "` for provenance/relationships, `" + brand.MCPToolName("wiki", "log") + "` for change history, and `" + brand.MCPToolName("wiki", "browse") + "` or `" + brand.MCPToolName("knowledge", "list") + "` for catalogues.",
		"",
