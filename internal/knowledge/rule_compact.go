...
func KnowledgeRuleContent(contexts []string, docsDir string) string {
	...
	return strings.Join([]string{
		...
		"",
		"1. Known slug → `" + brand.MCPToolName("wiki_source") + "`; otherwise `" + brand.MCPToolName("knowledge_search") + "` for the target/installed `context`, or `" + brand.MCPToolName("wiki_search") + "` across selected `wikis`/`hub_refs`. A cluster neighbor is Graphit-managed: use its returned `dir` as `project_dir` on wiki/module reads, not native discovery. Read its skill. For unresolved projects use Hub's `" + brand.MCPToolName("cluster_projects") + "` before its catalog. Hub-only: announce/install Knowledge; the question authorizes preparation. Without a local checkout, use resolved installed `id@version` as `context`, omitting `project_dir`. Public technologies need no Hub; verify version details in official docs.",
