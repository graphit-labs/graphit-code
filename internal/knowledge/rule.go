...
func KnowledgeRuleContent(contexts []string, docsDir string) string {
	...
	lines := []string{
		...
		"3. **Index the wiki** — run `" + binName + " sync &` (fire-and-forget).",
	...
	lines = append(lines,
		...
		"",
		"```bash",
		binName+` hub search "<system-name> --type knowledge"`,
		"```",
		"",
		"**Step 2 — If found, install it immediately:**",
		"",
		"```bash",
		binName+" knowledge import <artifact-name> --ide <antigravity|gemini|claude|cursor|kiro|codex|opencode>",
		"```",
		"",
		...
		"   - Run `"+binName+" hub search \"<system-name>\"` — always.",
		"   - If found: `"+binName+" knowledge import <name>` and read the wiki.",
...
func KnowledgeRouterContent(docsDir string) string {
	...
	lines := []string{
		...
		"3. **Reindex the wiki** — run `" + binName + " sync &` after any docs change.",
		"4. **Only then** report the task as complete.",
		"",
		...
		"",
		"> **Short version:** Code changed → read `" + knowledgeSkillName + "` skill → document → reindex → done.",
		"",
		...
		"- **Reindex after docs changes**: `" + binName + " sync &`",
		"- **Hub search before integration**: `" + binName + " hub search \"<system-name>\"`",
		"",
		...
		"2. Run `" + binName + " sync &` (fire-and-forget — reindexes AST + knowledge)",
		"3. If architectural change → update `docs/architecture/`",
		"4. If design decision → create ADR in `docs/decisions/`",
		"",
		"**A task without documentation is NOT complete. A task without `" + binName + " sync &` is NOT complete.**",
		"",
		...
		"2. **Definition of done**: code + documentation + task log + `" + binName + " sync &`. If any are missing, the task is NOT complete.",
		...
		"8. **Reindex after changes** — run `" + binName + " sync &` after every source file or docs modification.",
		...
		"   on every `" + binName + " init` and `" + binName + " sync`.",
		...
		"> files changed, and key decisions. Then run `" + binName + " sync &`.\"",
		"",
