...
func (t *EventTracker) TrackEvent(
	...
	anonymize := config.HubEventsAnonymize()
	secret, ok := eventClientSecret(anonymize)
	if !ok {
		return
	}
	addEventIdentities(payload, projectID, subject.UserID, anonymize, secret)
...
func eventClientSecret(anonymize bool) (string, bool) {
	if !anonymize {
		return "", true
	}
	secret, err := config.ClientSecret()
...
func addEventIdentities(payload map[string]any, projectID, userID string, anonymize bool, secret string) {
	delete(payload, "project_id")
	delete(payload, "user_id")
	delete(payload, "project_hash")
	delete(payload, "user_hash")
	...
	if projectID != "" {
		if hash := computeEventIdentityHash(projectID, secret); hash != "" {
			payload["project_hash"] = hash
		}
	}
	if userID != "" {
		if hash := computeEventIdentityHash(userID, secret); hash != "" {
			payload["user_hash"] = hash
		}
	}
}

func computeEventIdentityHash(identity, secret string) string {
	if identity == "" || secret == "" {
		return ""
	}
	h := sha256.Sum256([]byte(identity + secret))
