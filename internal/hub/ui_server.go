...
func NewUIServerOnPort(svc *HubService, agent string, port int) (*UIServer, error) {
	s := &UIServer{svc: svc, port: port, mux: http.NewServeMux(), agent: agent}
...
func (s *UIServer) resolveAgent(r *http.Request) string {
...
func (s *UIServer) handleRegistry(w http.ResponseWriter, r *http.Request) {
	...
	writeJSONUI(w, map[string]any{
		...
		"project_path":        projectDir, "agent": s.resolveAgent(r),
...
func (s *UIServer) handleProjectArtifacts(w http.ResponseWriter, r *http.Request) {
	...
	adapter := agent.GetAdapter(s.resolveAgent(r))
	if adapter != nil {
...
func (s *UIServer) handleInstall(w http.ResponseWriter, r *http.Request) {
	...
	_, err := s.svc.Install(r.Context(), installID, body.Alias, body.Agent, ArtifactType(body.Type), "", body.ProjectDir)
	if err != nil {
...
func (s *UIServer) handleUninstall(w http.ResponseWriter, r *http.Request) {
	...
	err := s.svc.Uninstall(r.Context(), id, ArtifactType(body.Type), false, body.Agent, body.ProjectDir)
...
func (s *UIServer) handleUpdateAll(w http.ResponseWriter, r *http.Request) {
	...
	results := s.svc.UpdateAll(r.Context(), body.Agent, body.ProjectDir)
...
func (s *UIServer) handleUpdateOne(w http.ResponseWriter, r *http.Request) {
	...
	err := s.svc.UpdateOne(r.Context(), body.ID, ArtifactType(body.Type), body.Agent, body.ProjectDir)
	if err != nil {
		writeJSONUI(w, map[string]any{"success": false, "error": err.Error()})
		return
	}
...
func (s *UIServer) handleSubmit(w http.ResponseWriter, r *http.Request) {
	...
	if body.ProjectDir != "" {
		agent := s.resolveAgent(r)
		if err := s.svc.RecordPublish(ctx, body.ID, ArtifactType(body.Type), body.Version, agent, body.ProjectDir); err != nil {
...
func (s *UIServer) handleUnpublish(w http.ResponseWriter, r *http.Request) {
	...
	{
		agent := s.resolveAgent(r)
		_ = s.svc.Uninstall(ctx, body.ID, ArtifactType(body.Type), true, agent, body.ProjectDir)
	}
...
func (s *UIServer) handleUpload(w http.ResponseWriter, r *http.Request) {
	...
	if recordDir != "" {
		agent := s.resolveAgent(r)
		if err := s.svc.RecordPublish(ctx, artifactID, ArtifactType(artType), version, agent, recordDir); err != nil {
...
func (s *UIServer) handleUnlink(w http.ResponseWriter, r *http.Request) {
	...
	if err := s.svc.Unlink(r.Context(), req.ID, agent, ArtifactType(req.Type), req.ProjectDir); err != nil {
