...
func newMetadataCache(cacheRoot string, cfg config.S3Config, subject hubaccess.Subject, authorizationRevision string) *metadataCache {
	return &metadataCache{
		root:     filepath.Join(cacheRoot, "cache", hubFingerprint(cfg), subjectFingerprint(subject), fingerprint(authorizationRevision)),
