...
func OnInit(ctx context.Context, registry *RegistryManager, agent, projectDir string) error {
	...
	_, _ = AddAgent(pp.LockFilePath, agent)
	...
	if registry.IsReady() {
		...
		if err == nil {
			for _, baseline := range baselines {
				...
				_, _ = svc.Install(ctx, entryID, "", agent, baseline.Type, "", "")
			}
		}
	}

	_ = syncAgentAdapter(agent, pp, lf)

	if registry.IsReady() {
		tracker := NewEventTracker(registry.Store())
		tracker.TrackEvent(ctx, "project.init", lf.Project.ID, nil, map[string]string{"agent": agent})
	}

	return nil
}

func OnUpdate(ctx context.Context, registry *RegistryManager, agent, projectDir string) error {
	svc := NewHubService(registry)
	pp := paths.GetPathsForProject(agent, projectDir)
	...
	if registry.IsReady() {
		...
		if lf != nil {
			...
			tracker := NewEventTracker(registry.Store())
			tracker.TrackEvent(ctx, "project.update", lf.Project.ID, nil, map[string]string{"agent": agent})
		}
	}

	_ = svc.UpdateAll(ctx, agent, projectDir)
...
func OnRemove(ctx context.Context, registry *RegistryManager, agent, projectDir string) error {
	svc := NewHubService(registry)
	pp := paths.GetPathsForProject(agent, projectDir)
	...
	if registry.IsReady() {
		tracker := NewEventTracker(registry.Store())
		tracker.TrackEvent(ctx, "project.remove", projectID, nil, map[string]string{"agent": agent})
	}

	remaining, _ := RemoveAgent(pp.LockFilePath, agent)

	if adapter, err := getAgentAdapter(agent); err == nil && adapter != nil {
	...
	if len(remaining) == 0 {
		...
		_ = svc.UninstallAll(ctx, agent, projectDir)
	}

	return nil
}

func SyncAgentAdapter(agent, projectDir string, lf *Lockfile) error {
	pp := paths.GetPathsForProject(agent, projectDir)
	return syncAgentAdapter(agent, pp, lf)
}

func syncAgentAdapter(agent string, pp *paths.ProjectPaths, lf *Lockfile) error {
	adapter, err := getAgentAdapter(agent)
...
// Agent adapters consume: artifact ID to type, path, version, hash and project ID.
//
...
func InstalledArtifacts(agent, projectDir string) (map[string]map[string]string, error) {
	pp := paths.GetPathsForProject(agent, projectDir)
	lf, err := LoadLockfile(pp.LockFilePath)
	if err != nil {
