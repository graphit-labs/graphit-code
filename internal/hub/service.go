...
func (s *HubService) Install(
	...
	if !globalInstall {
		pp = paths.GetPathsForProject(agent, projectDir)
	}
	...
	if s.registry.IsReady() && !mountArtifact {
		...
		default:
			...
			if artType != TypeRule && agent != "" && !globalInstall {
				targetPath, err := agentAdapter.ArtifactTypePath(pp.ActiveProjectDir, agent, string(artType), localID)
				if err == nil {
					if err := os.MkdirAll(filepath.Dir(targetPath), 0o755); err != nil {
						s.log().Warn("creating target dir", "artifact", localID, "error", err)
					}
					fm := agentAdapter.GetFileMode(agent, string(artType))
					if fm == "folder" {
						_ = os.RemoveAll(targetPath)
						if err := copyDir(cloneDir, targetPath); err != nil {
							s.log().Warn("copying artifact to Agent dir", "artifact", localID, "error", err)
						}
					} else {
	...
	for _, dep := range entry.Dependencies {
		...
		if _, err := s.Install(ctx, depIDVersioned, "", agent, dep.Type, realID, projectDir); err != nil {
...
// materialised copy from an Agent directory. A global install has none of that — the
...
func (s *HubService) Uninstall(
	...
	for _, memberID := range meta.Members {
		if err := s.Uninstall(ctx, memberID, "", false, agent, projectDir); err != nil {
	...
	if artType != TypeRule && !agentIndependentTypes[artType] && agent != "" {
		if targetPath, err := agentArtifactPath(pp.ActiveProjectDir, agent, string(artType), entryID); err == nil {
...
func (s *HubService) UpdateOne(ctx context.Context, entryID string, entryType ArtifactType, agent, projectDir string) error {
	pp := paths.GetPathsForProject(agent, projectDir)
	...
	if meta.Version != targetVersion {
		...
		if err := s.Uninstall(ctx, resolvedID, entryType, true, agent, projectDir); err != nil {
			return err
		}
		...
		_, err = s.Install(ctx, newID, meta.Alias, agent, entryType, "", projectDir)
		return err
	}
...
func (s *HubService) UpdateAll(ctx context.Context, agent, projectDir string) map[string]error {
	pp := paths.GetPathsForProject(agent, projectDir)
	...
	for artType, typeMap := range lf.Artifacts {
		for artID, meta := range typeMap {
			...
			result[artID] = s.UpdateOne(ctx, artID, artType, agent, projectDir)
		}
	}
	return result
}

func (s *HubService) UninstallAll(ctx context.Context, agent, projectDir string) error {
	pp := paths.GetPathsForProject(agent, projectDir)
	...
	for artType, typeMap := range lf.Artifacts {
		for artID := range typeMap {
			if err := s.Uninstall(ctx, artID, artType, true, agent, projectDir); err != nil {
...
func (s *HubService) Link(
	ctx context.Context,
	artifactName, sourcePath, agent string,
	artType ArtifactType,
	projectDir string,
) (*LinkResult, error) {
	...
	case TypeRule:
		...
		// RULE.md; it never creates an Agent-specific file or symlink.
	...
	case TypeMCP:
		...
		adapter := agentAdapter.GetAdapter(agent)
		if adapter == nil {
	...
	default:
		...
		sourcePath, err := agentArtifactPath(absSource, agent, string(artType), artifactName)
		if err != nil {
		...
		targetPath, err := agentArtifactPath(pp.ActiveProjectDir, agent, string(artType), artifactName)
		if err != nil {
...
func (s *HubService) Unlink(
	...
	default:
		if targetPath, err := agentArtifactPath(pp.ActiveProjectDir, agent, string(artType), artifactName); err == nil {
			_ = os.Remove(targetPath)
		}
	}
...
func agentArtifactPath(projectDir, agent, artifactType, artifactName string) (string, error) {
	return agentAdapter.ArtifactTypePath(projectDir, agent, artifactType, artifactName)
}
