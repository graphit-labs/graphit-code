...
func HubRuleContent() string {
	return strings.Join([]string{
		...
		"## Resolve, select, read",
		"",
		"Before installing context for an unfamiliar ecosystem project, choosing between artifacts or recovering unresolved discovery, read [discovery through useful evidence](references/discovery-cases.md). It includes filled local/global branches and failure outcomes. If the local resource is unavailable, call `" + brand.MCPToolName("module_skill") + "` with `module: hub`, `reference: references/discovery-cases.md` and the known `project_dir` when available. Reuse the reference across related calls.",
		"",
		"1. For a named ecosystem project, call `" + brand.MCPToolName("cluster_projects") + "` with the current initialized `project_dir`. It lists local project instances in that cluster, optionally filtered by `label`, not the published catalog. Use the selected absolute `dir` as `project_dir` for AST/Knowledge and authorized local work; never replace a known local path with an artifact ID. Local projects may also originate from or be published in Hub. Without an initialized current project, report local discovery unavailable instead of inventing a path.",
...
func MandateTrigger() string {
	return agent.ModuleMandateTrigger(
		...
		"Named system → `"+brand.MCPToolName("cluster_projects")+"` first, use local absolute `dir`; no scoped match → `"+brand.MCPToolName("hub_projects")+"`. Artifacts of any supported type → `"+brand.MCPToolName("hub_search")+"`/`"+brand.MCPToolName("hub_show")+"`: report matches, announce/install an obvious requested fit, ask only for real ambiguity/tradeoffs, then read/use the right module. Refine misses; distinguish service failure from empty results. Published artifacts imply no checkout; global `id@version` never replaces a local path. Known public technologies need no Hub lookup; honor explicit Hub requests. Installing runnable content does not authorize execution. Configuration → `"+brand.MCPToolName("config_get")+"`.",
		nil, nil,
	)
}
