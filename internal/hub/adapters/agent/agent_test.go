...
func TestGetAdapter(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()
			got := GetAdapter(tc.agent)
			if (got == nil) != tc.wantNil {
				t.Errorf("GetAdapter(%q) nil=%v, want nil=%v", tc.agent, got == nil, tc.wantNil)
			}
		})
	}
}
...
func TestSupportedAgents(t *testing.T) {
	...
	for _, agent := range agents {
		if !expected[agent] {
			t.Errorf("unexpected Agent in list: %q", agent)
		}
		delete(expected, agent)
	}
...
func TestGetFileMode(t *testing.T) {
	...
	tests := []struct {
		...
		{"codex", "command", "file"},
		{"opencode", "agent", "file"},
		{"unknown-agent", "rule", "file"},
		{"gemini", "nonexistent-type", "file"},
	}
	for _, tc := range tests {
		t.Run(tc.agent+"/"+tc.artType, func(t *testing.T) {
			t.Parallel()
			got := GetFileMode(tc.agent, tc.artType)
			if got != tc.wantMode {
				t.Errorf("GetFileMode(%q, %q) = %q, want %q", tc.agent, tc.artType, got, tc.wantMode)
			}
		})
	}
}
...
func TestArtifactTypePath(t *testing.T) {
	...
	tests := []struct {
		...
		{"rules have no Agent path", "gemini", "rule", "my-rule", "", true},
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()
			got, err := ArtifactTypePath(dir, tc.agent, tc.artType, tc.artName)
			if tc.wantErr {
				if err == nil {
...
func TestNewRequestedAgentAdapters(t *testing.T) {
	t.Parallel()
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()
			base, ok := folderBase(GetAdapter(tc.name))
			if !ok {
				t.Fatalf("%s is not folder-backed", tc.name)
			}
			if base.cfg.RootDirName != tc.root || base.cfg.SkillsDir != tc.skills {
				t.Fatalf("%s config = root %q skills %q", tc.name, base.cfg.RootDirName, base.cfg.SkillsDir)
			}
		})
	}
}
...
func TestAdaptersInstallHubMCPAtTheNarrowestSupportedScope(t *testing.T) {
	...
	t.Setenv("GRAPHIT_GLOBAL_DIR", filepath.Join(homeDir, ".graphit"))
...
func TestFolderBasedAdapter_ScanLocal(t *testing.T) {
	...
	baseDir := filepath.Join(dir, ".test-agent")
...
func TestFolderBasedAdapter_ScanLocal_EmptyDir(t *testing.T) {
	t.Parallel()
	dir := t.TempDir()
	a := NewFolderBasedAdapter(FolderConfig{RootDirName: ".agent"})
	results := a.ScanLocal(dir)
	if len(results) != 0 {
...
func TestFolderBasedAdapter_Sync(t *testing.T) {
	...
	ruleFile := filepath.Join(dir, ".test-agent", "rules", "rule1.md")
	if _, err := os.Stat(ruleFile); !os.IsNotExist(err) {
		t.Errorf("rule artifact was materialized: %v", err)
	}

	skillDir := filepath.Join(dir, ".test-agent", "skills", "skill1")
	if _, err := os.Stat(skillDir); err != nil {
		t.Errorf("skill dir not created: %v", err)
	}

	cmdFile := filepath.Join(dir, ".test-agent", "commands", "cmd1.md")
...
func TestFolderBasedAdapter_Sync_Workflow(t *testing.T) {
	...
	wfFile := filepath.Join(dir, ".test-agent", "commands", "wf1.md")
...
func TestFolderBasedAdapter_Remove_NilInstalledPreservesUserDirectory(t *testing.T) {
	t.Parallel()
	dir := t.TempDir()
	a := NewFolderBasedAdapter(FolderConfig{RootDirName: ".test-agent"})
...
func TestInstallManagedSkill(t *testing.T) {
	...
	t.Run("install for known Agent", func(t *testing.T) {
		t.Parallel()
		dir := t.TempDir()
	...
	t.Run("install for each adapter type", func(t *testing.T) {
		...
		for _, agent := range SupportedAgents() {
			t.Run(agent, func(t *testing.T) {
				t.Parallel()
				dir := t.TempDir()
				err := InstallManagedSkill(dir, agent, "s1", "content")
				if err != nil {
					t.Fatalf("install for %s failed: %v", agent, err)
				}
			})
		}
	})
	...
	t.Run("unknown Agent", func(t *testing.T) {
		t.Parallel()
		dir := t.TempDir()
		err := InstallManagedSkill(dir, "no-such-agent", "s1", "content")
		if err == nil {
...
func TestRemoveManagedSkill(t *testing.T) {
	...
	t.Run("remove for each adapter type", func(t *testing.T) {
		...
		for _, agent := range SupportedAgents() {
			t.Run(agent, func(t *testing.T) {
				t.Parallel()
				dir := t.TempDir()
				_ = InstallManagedSkill(dir, agent, "s1", "content")
				err := RemoveManagedSkill(dir, agent, "s1")
				if err != nil {
					t.Fatalf("remove for %s failed: %v", agent, err)
				}
			})
		}
	})
	...
	t.Run("unknown Agent", func(t *testing.T) {
		t.Parallel()
		dir := t.TempDir()
		err := RemoveManagedSkill(dir, "no-such-agent", "s1")
		if err == nil {
			t.Error("expected error for unknown Agent")
		}
	})
...
func TestFolderBasedAdapter_BaseDir(t *testing.T) {
	...
	t.Run("with root dir", func(t *testing.T) {
		...
		want := filepath.Join("/project", ".agent")
		if got != want {
			t.Errorf("got %q, want %q", got, want)
		}
	})
...
func TestFolderBasedAdapter_Sync_AgentToRulesDir(t *testing.T) {
	...
	agentFile := filepath.Join(dir, ".test-agent", "rules", "agent1_agent.md")
