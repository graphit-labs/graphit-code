...
func GetAdapter(agent string) Adapter {
...
// `description`, which is what an Agent matches a request against.
//
...
// The skill is then not degraded, it is invisible: the Agent discovers no valid
...
// Both fields are validated against the limits every Agent enforces on a skill,
...
func validateSkillDescription(name, description string) error {
	if strings.TrimSpace(description) == "" {
		return fmt.Errorf("skill %s: description is empty, and it is what the Agent matches a request against", name)
	}
...
func InstallManagedSkill(projectDir, agentName, skillName, content string) error {
...
func RemoveManagedSkill(projectDir, agentName, skillName string) error {
	adapter := GetAdapter(agentName)
	if adapter == nil {
		return fmt.Errorf("unknown Agent: %s", agentName)
	}
...
func ArtifactTypePath(projectDir, agentName, artifactType, artifactName string) (string, error) {
	if artifactType == "rule" {
		return "", fmt.Errorf("rule artifacts are delivered dynamically by lifecycle hooks and have no Agent path")
	}
	adapter := GetAdapter(agentName)
	if adapter == nil {
		return "", fmt.Errorf("unknown Agent: %s", agentName)
	...
	if !ok {
		return "", fmt.Errorf("unsupported adapter type for Agent %s", agentName)
	}
	rootDir := baseAdapter.cfg.RootDirName
	typeDir := baseAdapter.getTypeDir(artifactType)
	fm := baseAdapter.getFileMode(artifactType)
	if typeDir == "" {
		return "", fmt.Errorf("unknown artifact type %q for Agent %s", artifactType, agentName)
	}
...
func GetFileMode(agentName, artifactType string) string {
	adapter := GetAdapter(agentName)
	if adapter == nil {
		return "file"
	}
