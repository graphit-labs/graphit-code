...
func mandatePreamble() string {
	return strings.Join([]string{
		...
		// Graphit owns work semantics; each host owns agent turn/resume mechanics.
		"Subagent default: create none unless the user or an applicable mandate, AGENTS.md, loaded skill or higher-priority instruction explicitly assigns bounded work to a delegated role; that instruction authorizes and requires only that role and work. This mandate assigns recall→`" + brand.Brand + "-scout`, impact review→`" + brand.Brand + "-tracker`, and decided transcription→`" + brand.Brand + "-scribe`. A performer already in that role works directly, not recursively. Delegates report evidence and may finish; no waiting loop. The coordinator may later send that delegate another instruction through host follow-up/resume when supported. Reuse is optional: no keep-alive, reuse or explicit dismissal is required, and ending a turn does not complete Graphit work. Acceptance, checkpoints, Task/session claims, revisions, completion and lifecycle stay with the coordinator. If the host cannot run the role, read its agents-directory document and perform only that role yourself.",
	}, "\n") + "\n"
}
