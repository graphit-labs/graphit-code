...
func NewFolderBasedAdapter(cfg FolderConfig) *FolderBasedAdapter {
	...
	if cfg.RulesDir == "" && !cfg.UnsupportedTypes["rule"] {
		cfg.RulesDir = "rules"
	}
	if cfg.CommandsDir == "" && !cfg.UnsupportedTypes["command"] {
		cfg.CommandsDir = "commands"
	}
	if cfg.SkillsDir == "" && !cfg.UnsupportedTypes["skill"] {
...
func (a *FolderBasedAdapter) Sync(
	...
	for _, eid := range ids {
		...
		artType := edata["type"]
		localName := eid
		normalizedType := artType
		if normalizedType == "workflow" {
			normalizedType = "command"
		}
		if a.cfg.UnsupportedTypes[normalizedType] {
			continue
		}
...
func (a *FolderBasedAdapter) getTypeDir(artType string) string {
	...
	if a.cfg.UnsupportedTypes[artType] {
		return ""
	}
	switch artType {
	case "rule":
