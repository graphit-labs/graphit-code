...
func TestSyncAgentAdapter_InvalidAgent(t *testing.T) {
	t.Parallel()
	dir := t.TempDir()
	...
	err := syncAgentAdapter("totally-invalid-agent-12345", pp, lf)
	if err == nil {
		t.Error("expected error for unsupported Agent")
	}
}

func TestGetAgentAdapter(t *testing.T) {
	t.Parallel()

	t.Run("unsupported Agent", func(t *testing.T) {
		t.Parallel()
		_, err := getAgentAdapter("totally-unsupported-agent-xyz")
		if err == nil {
			t.Error("expected error for unsupported Agent")
		}
	})

	t.Run("supported Agent", func(t *testing.T) {
		t.Parallel()
		adapter, err := getAgentAdapter("claude")
		if err != nil {
			t.Logf("claude Agent may not be supported in this environment: %v", err)
			return
		}
		if adapter == nil {
