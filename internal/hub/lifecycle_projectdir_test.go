...
func TestOnRemoveTargetsTheProjectItWasGivenNotTheWorkingDirectory(t *testing.T) {
	...
	target := t.TempDir()
	pp := paths.GetPathsForProject(agent, target)
	...
	if _, err := AddAgent(pp.LockFilePath, agent); err != nil {
	...
	if len(before.Agents) == 0 {
		t.Fatalf("seed did not record the Agent: %+v", before.Agents)
	}

	if err := OnRemove(context.Background(), &RegistryManager{}, agent, target); err != nil {
	...
	if after != nil {
		for _, got := range after.Agents {
			if got == agent {
				t.Errorf("the Agent is still registered in %s — OnRemove did not act on the "+
...
// UninstallAll is reached only when the removed Agent was the last one, which is the
// path that resolved an empty project to the working directory. Seeded with a
// single Agent so the removal empties the list and that branch runs.
...
func TestSyncAgentAdapterTargetsTheGivenProjectNotTheWorkingDirectory(t *testing.T) {
	cwd := t.TempDir()
	target := t.TempDir()
	...
	for _, agent := range []string{"claude", "codex"} {
		if err := SyncAgentAdapter(agent, target, lf); err != nil {
			t.Fatalf("sync %s adapter: %v", agent, err)
		}
	}
...
func TestSyncAgentAdapterKeepsMCPAndHooksMachineIndependent(t *testing.T) {
	...
	tests := []struct {
		...
		{agent: "antigravity", hookPath: filepath.Join(".agents", "hooks.json"), mcpPath: filepath.Join(".agents", "mcp_config.json")},
		{agent: "cursor", hookPath: filepath.Join(".cursor", "hooks.json"), mcpPath: filepath.Join(".cursor", "mcp.json")},
		{agent: "claude", hookPath: filepath.Join(".claude", "settings.json"), mcpPath: ".mcp.json"},
		{agent: "kiro", hookPath: filepath.Join(".kiro", "hooks", "graphit-memory.json"), mcpPath: filepath.Join(".kiro", "settings", "mcp.json")},
		{agent: "codex", hookPath: filepath.Join(".codex", "hooks.json"), mcpPath: filepath.Join(".codex", "config.toml")},
		{agent: "opencode", hookPath: filepath.Join(".opencode", "plugins", "graphit-memory-session-start.js"), mcpPath: "opencode.json"},
		{agent: "gemini", hookPath: filepath.Join(".gemini", "settings.json"), mcpPath: filepath.Join(".gemini", "settings.json")},
	}
	...
	for _, tc := range tests {
		t.Run(tc.agent, func(t *testing.T) {
			...
			lf := &Lockfile{
				Project:   ProjectIdentity{ID: "sync-mcp-hooks-" + tc.agent},
				Artifacts: make(map[ArtifactType]map[string]*LockfileArtifactMeta),
			}
			...
			if err := SyncAgentAdapter(tc.agent, projectDir, lf); err != nil {
...
func TestOnInitRegistersProjectInGlobalLock(t *testing.T) {
	...
	if err := OnInit(context.Background(), reg, agent, target); err != nil {
