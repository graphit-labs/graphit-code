...
func ImprovementsRouterContent() string {
	lines := []string{
		...
		"",
		"- **Add via CLI**: `" + brand.BinName() + " dream subject add \"Title\" --body \"Detailed instructions\"`",
		"- **Add via file**: Create `" + brand.DotDir() + "/dream/subjects/<slug>.md` with a `# Title` heading and instructions",
		"- **List subjects**: `" + brand.BinName() + " dream subject list`",
		"- **Remove**: `" + brand.BinName() + " dream subject rm <slug>`",
		"- **Completion**: The dream agent creates `<slug>.done.md` when finished",
		"",
		"## 🔗 Subagent Improvements",
		"",
		"**When spawning subagents, include in their prompt:**",
		"\"If you notice code patterns that could be improved but are outside your current scope, create a dream subject:",
		"`" + brand.BinName() + " dream subject add \"<improvement title>\" --body \"<detailed description of what to improve and why>\"` \"",
		"",
		"## ⛔ Critical Rules",
		"",
