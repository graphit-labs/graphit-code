...
func RuleContent(contexts []string) string {
	...
	return strings.Join([]string{
		...
		"",
		"Use the questioned project's known/cluster-returned `dir` as `project_dir` with project scope, including neighbors; user scope is for cross-project preferences. If unresolved, read Hub and resolve with `" + brand.MCPToolName("cluster_projects") + "` before its catalog; never guess paths or read stores as files. Installed AST/Knowledge contexts provide no project memories. For missing context, refine once; use `" + brand.MCPToolName("memory_list") + "` for an intended inventory or suspected empty store, not routine recall or post-write verification.",
		"",
