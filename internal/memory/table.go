...
//
// The field list is the complete persistence contract for both current and historical records.
...
func memoryTableIndexes() []lancestore.Index {
	return []lancestore.Index{
		{Column: "body", Kind: lancestore.IndexInvertedText},
		{Column: "title", Kind: lancestore.IndexInvertedText},
		{Column: "tags_json", Kind: lancestore.IndexInvertedText},
		{Column: "type", Kind: lancestore.IndexInvertedText},
...
const memoryMinRowsForVectorIndex = 256
...
func OpenMemoryTable(ctx context.Context, uri string) (*MemoryTable, error) {
	s3cfg := memoryS3ConfigForURI(ctx, uri)
	if err := authorizeMemoryURI(ctx, uri, s3cfg); err != nil {
		return nil, err
	}
...
func authorizeMemoryURI(ctx context.Context, uri string, configs ...config.S3Config) error {
	if !strings.HasPrefix(uri, "s3://") {
		return nil
	}
	cfg := memoryS3ConfigForURI(ctx, uri)
	if len(configs) > 0 {
		cfg = configs[0]
	}
...
// List reads every record, live memories first and then archived revisions. Each group follows the
// canonical Memory order: mandatory, important, normal, then newest first.
...
// RefreshIndexes makes records written since the last maintenance pass visible to full-text
// search. The indexes live on the authoritative table; no derived query store is involved.
func (t *MemoryTable) RefreshIndexes(ctx context.Context) error {
	if err := t.EnsureIndexes(ctx); err != nil {
		return err
	}
	embedded, err := t.countEmbedded(ctx)
	if err != nil {
		return err
	}
	if embedded >= memoryMinRowsForVectorIndex {
		if err := t.table.EnsureIndexes(ctx, lancestore.Index{Column: "embedding", Kind: lancestore.IndexVectorIVFPQ}); err != nil {
			return err
		}
	}
	return t.table.FoldNewRowsIntoIndexes(ctx)
}

// PendingEmbeddings returns records whose vector has not been generated yet.
func (t *MemoryTable) PendingEmbeddings(ctx context.Context) ([]MemoryRecord, error) {
	hits, err := t.table.Search(ctx, lancestore.Query{
		Filter: "embedding IS NULL", Limit: 100000,
	})
	if err != nil {
		return nil, err
	}
	...
	for _, hit := range hits {
		out = append(out, recordFromRow(hit.Row))
	}
	return out, nil
}

// SetEmbedding attaches a vector to one current or archived memory record.
func (t *MemoryTable) SetEmbedding(ctx context.Context, key string, vec []float32) error {
	if want := ai.ResolveConfiguredEmbeddingDimensions(); len(vec) != want {
		return fmt.Errorf("memory embedding for %s has %d dimensions, want %d", key, len(vec), want)
	}
	...
	if !ok {
		return fmt.Errorf("memory table has no record %q to attach an embedding to", key)
	}
	rec.Embedding = vec
	return t.Put(ctx, rec)
}

func (t *MemoryTable) countEmbedded(ctx context.Context) (int, error) {
	hits, err := t.table.Search(ctx, lancestore.Query{
		Filter: "embedding IS NOT NULL", Columns: []string{"key"}, Limit: 100000,
	})
	return len(hits), err
}
...
func recordFromMarkdown(rel string, data []byte) (MemoryRecord, bool) {
	...
	superseded := isHistoryKey(rel) || fm.IsArchivedRevision()
