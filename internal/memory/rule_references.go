...
func SkillReferences() map[string]string {
	return map[string]string{
		"references/durable-context.md": strings.NewReplacer(
			...
			"TOOL_DELETE", brand.MCPToolName("memory_delete"),
		).Replace(memoryDurableContext),
	}
}
...
Suppose it identifies current memory 01J00000000000000000000004, “Ecosystem discovery,” whose title alone does not reveal its conditions. Read TOOL_SOURCE:
~~~json
{"project_dir":"/work/portal","scope":"project","id":"01J00000000000000000000004"}
~~~

Assume its current content says every external dependency must first be found in Hub, while also requiring exact version selection and resolving local project paths at call time. The first statement is superseded; the latter conditions still matter. Update the same subject with TOOL_UPDATE:
...
If no current matching subject exists, TOOL_INSERT can store:
~~~json
{
  "project_dir":"/work/portal",
  "scope":"project",
...
Do not delete B because its title looks redundant. Merge the distinct conditions, rationale and provenance into A with TOOL_UPDATE. Read A once with TOOL_SOURCE to verify preservation before deleting B with TOOL_DELETE. If preservation is uncertain, keep both and record the conflict. Routine writes do not require this readback; deletion after a merge does.
