...
func loadMemoryVectors(records []MemoryRecord) map[string][]float32 {
	byID := make(map[string][]float32, len(records))
	for _, record := range records {
		if !record.Superseded && len(record.Embedding) > 0 {
			byID[record.ID] = record.Embedding
		}
	}
	return byID
}
