...
func RunConsolidation(ctx context.Context, scope string, aiClient ai.Client) (*ConsolidationReport, error) {
	return RunConsolidationInDir(ctx, scope, "", aiClient)
}

// RunConsolidationInDir keeps every AI batch in one native agent session rooted
// at workDir. The scope still controls which authoritative table is read.
func RunConsolidationInDir(ctx context.Context, scope, workDir string, aiClient ai.Client) (*ConsolidationReport, error) {
	...
	if aiClient != nil {
		aiClient = ai.NewConversation(aiClient, workDir)
	}
	return consolidateSnapshots(ctx, memories, aiClient, loadMemoryVectors(records)), nil
}
