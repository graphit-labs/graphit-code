...
func (m *MemoryService) AddMemory(title, body string, opts MemoryOpts) (string, error) {
	...
	if err := tbl.RefreshIndexes(ctx); err != nil {
		m.log().Warn("post-write index refresh failed", "op", "add", "id", id, "error", err)
	}
	return id, nil
}
...
func (m *MemoryService) updateMemory(id, newTitle, newBody, memType string) error {
	...
	if err := tbl.RefreshIndexes(ctx); err != nil {
		m.log().Warn("post-write index refresh failed", "op", "update", "id", id, "error", err)
	}
	return nil
}
...
func (m *MemoryService) RemoveMemory(id string) error {
	...
	if err := tbl.RefreshIndexes(ctx); err != nil {
		m.log().Warn("post-write index refresh failed", "op", "remove", "id", id, "error", err)
	}
	return nil
}
...
func (m *MemoryService) ListMemories() ([]MemoryEntry, error) {
	...
	SortMemoryEntries(memories)
	return memories, nil
}

// MemoryTrace is the complete authoritative revision chain for one memory identity. Current is nil
// after deletion, while Revisions remains available for provenance and audit views.
type MemoryTrace struct {
	Current   *MemoryRecord
	Revisions []MemoryRecord
}

// TraceMemory reads a memory and every archived revision directly from the authoritative table.
func (m *MemoryService) TraceMemory(ctx context.Context, id string) (MemoryTrace, bool, error) {
	tbl, err := m.openTable(ctx)
	if err != nil {
		return MemoryTrace{}, false, err
	}
	defer func() { _ = tbl.Close() }()

	current, live, err := tbl.Get(ctx, id)
	if err != nil {
		return MemoryTrace{}, false, err
	}
	revisions, err := tbl.Revisions(ctx, id)
	if err != nil {
		return MemoryTrace{}, false, err
	}
	trace := MemoryTrace{Revisions: revisions}
	if live {
		trace.Current = &current
	}
	return trace, live || len(revisions) > 0, nil
}
...
// IndexMemories ensures the authoritative table's own search indexes are present and current.
func (m *MemoryService) IndexMemories(ctx context.Context) error {
	...
	return tbl.RefreshIndexes(ctx)
}

// ReadMemory returns the rendered current record or archived revision addressed by key.
func (m *MemoryService) ReadMemory(ctx context.Context, key string) (string, bool, error) {
	tbl, err := m.openTable(ctx)
	if err != nil {
		return "", false, err
	}
	defer func() { _ = tbl.Close() }()
	return m.readMarkdown(ctx, tbl, normalizeMemoryKey(key))
}
