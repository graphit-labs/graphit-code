...
export default function WikiExplorerPage({ autoSelectProject }: WikiExplorerProps = {}) {
  ...
  const explorerBase = location.pathname.startsWith('/knowledge/')
    ? '/knowledge/explorer'
    : '/wiki/explorer'
  ...
  }, [autoSelectProject, moduleIdFromURL, activeProjectDir])
  ...
  const runSearch = useCallback(async () => {
    ...
    if (aiRequested) {
      ...
      setView('ai-search')
      try {
        const resp = await aiSearchWiki(selectedModule.path, searchQ, activeProjectDir || undefined)
        setAiResponse(resp)
      } catch {
  ...
  }, [selectedModule, searchQ, searchMode, aiEnabled, activeProjectDir])
          ...
          {!leftCollapsed ? (
            <button
              onClick={() => {
                ...
                else navigate('/knowledge/contexts')
              }}
  ...
  return (
    <div className="flex flex-col items-center justify-center py-20 text-center gap-5 bg-card/35 border border-border/30 backdrop-blur-md rounded-2xl p-8">
      ...
      <div className="flex flex-col gap-2 text-left mt-3 w-full max-w-sm">
        {['graphit knowledge index docs/'].map(cmd => (
