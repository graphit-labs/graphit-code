...
export default function ProjectArtifactsPage() {
  const { activeAgent, webMode, activeProjectDir, projectName, setProjectName } = useAppStore()
  ...
  const load = useCallback(async () => {
    setLoading(true)
    try {
      const data = await hubApi.getProjectArtifacts(activeProjectDir, activeAgent)
  ...
  const handleUnlink = async (art: InstalledArtifact) => {
    try {
      await hubApi.unlinkLocal(art.local_id, art.type, activeAgent, projectPath)
  ...
  const handleRemove = async () => {
    ...
    try {
      const res = await hubApi.uninstall(art.local_id, art.local_id, activeAgent, art.type, activeProjectDir)
  ...
  const handleUpdate = async (id: string, type: string) => {
    try {
      const res = await hubApi.updateOne(id, type, activeAgent, activeProjectDir)
      if (res.success) {
    ...
    <div className="w-full max-w-7xl mx-auto px-1 sm:px-2 lg:px-4 py-8 lg:py-12 animate-in fade-in duration-300">
      ...
      <div className="flex items-center justify-between gap-4 mb-8 pb-6 border-b border-border/40">
        <div className="flex items-center gap-4">
          ...
          <div>
            <p className="font-mono text-[10px] uppercase tracking-[0.18em] text-primary font-semibold mb-1">Hub / installed</p>
      ...
      {loading ? (
        ...
        <EmptyState
          ...
          description="Create artifacts in your Agent directory (rules, skills, agents, etc.) or install them from the Registry."
        />
      ) : (
        <div className="space-y-8">
          ...
          <section>
            ...
            <div className="space-y-8">
              ...
              {}
              <div>
                <h3 className="text-sm font-semibold text-muted-foreground mb-4 uppercase tracking-wider">Agent Artifacts</h3>
                {!projectArtifacts.some(a => !['knowledge', 'ast', 'language', 'framework'].includes(a.type)) ? (
                  <div className="rounded-xl border border-dashed border-border p-6 text-center bg-muted/20">
                    <p className="text-sm text-muted-foreground">
                      No Agent-specific artifacts found. Create rules, skills, agents, or commands in your Agent directory (e.g. <code className="text-xs bg-muted px-1.5 py-0.5 rounded">.agents/</code> or <code className="text-xs bg-muted px-1.5 py-0.5 rounded">.gemini/</code>) to see them here and submit them to the Hub.
                    </p>
                  </div>
                ) : (
                  <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
