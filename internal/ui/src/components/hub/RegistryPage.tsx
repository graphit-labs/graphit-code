...
export default function RegistryPage() {
  const {
    ...
    activeAgent, setActiveAgent, setActiveProjectId, activeProjectId, webMode,
  ...
  }, [activeProjectDir, activeAgent, webMode, setActiveAgent, setActiveProjectId, projectName])
  ...
  const doInstall = async (id: string, type: string, alias: string | null, version?: string) => {
    ...
    try {
      const res = await hubApi.install(id, alias, activeAgent, type, activeProjectDir, version)
      if (res.success) {
  ...
  const doUninstall = async () => {
    ...
    try {
      const res = await hubApi.uninstall(entry.id, inst.local_id, activeAgent, entry.type, activeProjectDir)
      if (res.success) {
