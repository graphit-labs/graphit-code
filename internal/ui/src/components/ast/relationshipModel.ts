...
export function observedGraph(nodes: GraphNode[], links: GraphEdge[]) {
  ...
  for (const e of links) {
    if (byId.has(e.source) && byId.has(e.target)) unique.set(JSON.stringify([e.source, e.type, e.target]), e);
  }
  return { nodes: [...byId.values()].sort(byName), links: [...unique.values()].sort((a,b) => a.source.localeCompare(b.source) || a.type.localeCompare(b.type) || a.target.localeCompare(b.target)), byId };
}
