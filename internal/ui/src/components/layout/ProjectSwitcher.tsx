...
export function ProjectSwitcher() {
  const {
    projects, activeProjectDir, projectName, activeAgent,
    switchProject, setActiveAgent, projectsLoaded, supportedAgents,
  } = useAppStore()
  ...
  const currentAgentLabel = capitalizeAgent(activeAgent)

  return (
    <div className="px-3.5 py-3 border-b border-white/[0.08] space-y-1.5 relative z-20">
      <div ref={projectRef} className="relative">
        <button
          onClick={() => { setProjectOpen((v) => !v); setAgentOpen(false) }}
      ...
      <div ref={agentRef} className="relative">
        <button
          onClick={() => { setAgentOpen((v) => !v); setProjectOpen(false) }}
        ...
        >
          ...
          <span className="text-[11px] text-white/38 font-medium flex-1 text-left">Agent <span className="text-white/72 font-semibold ml-1">{currentAgentLabel}</span></span>
          <ChevronDown className={cn(
            ...
            {supportedAgents.map((agentId) => (
              <button
                ...
                className={cn(
                  ...
                  agentId === activeAgent && 'bg-[#b9fb63]/10',
                )}
              >
                <span className="text-sm font-medium text-white">{capitalizeAgent(agentId)}</span>
                {agentId === activeAgent && (
