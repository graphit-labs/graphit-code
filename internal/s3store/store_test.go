...
func TestStoreRefreshesTemporaryCredentialsBeforeFirstSignedRequest(t *testing.T) {
	fake, endpoint := testsupport.StartFakeS3(t, "graphit-hub")
	calls := 0
	base := config.S3Config{Bucket: "graphit-hub", Region: "us-east-1", Endpoint: endpoint, Prefix: "user/alice", AccessKeyID: "expired-key", SecretAccessKey: "expired-secret", SessionToken: "expired-token", ExpiresAt: time.Now().Add(time.Minute)}
	base.Refresh = func(context.Context) (config.S3Config, error) {
		calls++
		refreshed := base
		refreshed.AccessKeyID, refreshed.SecretAccessKey, refreshed.SessionToken = "renewed-key", "renewed-secret", "renewed-token"
	...
	if calls != 1 || fake.LastAccessKey() != "renewed-key" {
		t.Fatalf("refresh calls=%d signed key=%q", calls, fake.LastAccessKey())
	}
}

func TestTemporaryCredentialRefreshRejectsTopologyChange(t *testing.T) {
	base := config.S3Config{Bucket: "one", Region: "us-east-1", AccessKeyID: "A", SecretAccessKey: "S"}
	base.Refresh = func(context.Context) (config.S3Config, error) {
		changed := base
		changed.Bucket = "two"
		return changed, nil
	}
	if _, err := (&refreshingCredentialsProvider{initial: base}).Retrieve(context.Background()); err == nil {
		t.Fatal("credential refresh accepted a bucket change")
	}
}
