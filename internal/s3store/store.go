...
func New(ctx context.Context, cfg config.S3Config) (*Store, error) {
	if cfg.ResolutionError != nil {
		return nil, fmt.Errorf("resolve active account S3 configuration: %w", cfg.ResolutionError)
	}
	if !cfg.Configured() {
	...
	if cfg.HasStaticCredentials() {
		provider := &refreshingCredentialsProvider{initial: cfg}
		opts = append(opts, awsconfig.WithCredentialsProvider(aws.NewCredentialsCache(provider, func(options *aws.CredentialsCacheOptions) {
			options.ExpiryWindow = 2 * time.Minute
...
func (p *refreshingCredentialsProvider) Retrieve(ctx context.Context) (aws.Credentials, error) {
	...
	if cfg.Refresh != nil {
		...
		if err != nil {
			return aws.Credentials{}, fmt.Errorf("refresh Graphit S3 credentials: %w", err)
		}
		if refreshed.Bucket != cfg.Bucket || refreshed.Prefix != cfg.Prefix || refreshed.Region != cfg.Region || refreshed.Endpoint != cfg.Endpoint {
			return aws.Credentials{}, errors.New("S3 topology changed while refreshing credentials; rebuild the store")
		}
		cfg = refreshed
	}
	if !cfg.HasStaticCredentials() {
		return aws.Credentials{}, errors.New("refreshed S3 grant omitted access key or secret key")
	}
	credentials := aws.Credentials{
		AccessKeyID: cfg.AccessKeyID, SecretAccessKey: cfg.SecretAccessKey, SessionToken: cfg.SessionToken,
		Source: "graphit-s3-grant",
	}
	if !cfg.ExpiresAt.IsZero() {
		credentials.CanExpire = true
		credentials.Expires = cfg.ExpiresAt
	}
