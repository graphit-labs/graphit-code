...
// must notice.
const CanonicalManifestVersion = 3

// CanonicalLangCount materializes one node table's language distribution while the
// exporter already has its rows in memory. Consumers can then render schema statistics
// without scanning every node through the graph engine.
type CanonicalLangCount struct {
...
type CanonicalNodeTable struct {
	...
	LangCounts []CanonicalLangCount `json:"lang_counts"`
}
...
// optional `<member>_reverse` mirrors, schema.cypher, and the v3 manifest mapping logical
...
func exportCanonicalNodes(c Conn, outDir, label string) (*CanonicalNodeTable, map[int64]uint64, error) {
	...
	return &CanonicalNodeTable{
		...
		PrimaryKey: pk,
		Columns:    cols,
		LangCounts: sortedCanonicalLangCounts(langCounts),
	}, ids, nil
}

func sortedCanonicalLangCounts(counts map[string]int64) []CanonicalLangCount {
	langs := make([]string, 0, len(counts))
	for lang := range counts {
		langs = append(langs, lang)
	}
	sort.Strings(langs)
	out := make([]CanonicalLangCount, 0, len(langs))
	for _, lang := range langs {
		out = append(out, CanonicalLangCount{Lang: lang, Rows: counts[lang]})
	}
	return out
}
