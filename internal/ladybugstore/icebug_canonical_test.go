...
func TestIcebugCanonicalRoundTrip(t *testing.T) {
	...
	for _, table := range man.NodeTables {
		if len(table.LangCounts) != 1 {
			t.Fatalf("%s language stats = %#v, want one complete bucket", table.Label, table.LangCounts)
		}
		wantLang := ""
		if table.Label == "File" {
			wantLang = "go"
		}
		if table.LangCounts[0].Lang != wantLang || table.LangCounts[0].Rows != table.Rows {
			t.Fatalf("%s language stats = %#v, want %q x%d", table.Label, table.LangCounts, wantLang, table.Rows)
		}
	}
