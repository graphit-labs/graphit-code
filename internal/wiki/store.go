...
func (w *WikiDB) SearchWithOptions(ctx context.Context, query string, topK int, opts WikiSearchOptions) ([]WikiSearchResult, error) {
	...
	body, err := w.search(ctx, lancestore.Query{Text: text, TextColumn: lanceWikiBody, Filter: filter, Limit: limit, Rerank: rerank})
	if err != nil {
		return nil, err
	}
	terms, err := w.search(ctx, lancestore.Query{Text: text, TextColumn: lanceWikiTerms, Filter: filter, Limit: limit, Rerank: rerank})
	if err != nil {
		return nil, err
	}
...
func (w *WikiDB) HybridSearch(ctx context.Context, query string, vec []float32, topK int) ([]WikiSearchResult, error) {
	...
	bodyVector, err := w.search(ctx, lancestore.Query{
		...
		Vector: vec, VectorColumn: lanceWikiVector, Limit: limit, Rerank: rerank,
	})
	if err != nil {
		return nil, err
	}
	terms, err := w.search(ctx, lancestore.Query{Text: text, TextColumn: lanceWikiTerms, Limit: limit, Rerank: rerank})
	if err != nil {
		return nil, err
	}
...
// want one point per document, so only the first chunk contributes.
