...
func TestAuthRoundTripperRenewsBearerForEveryRequest(t *testing.T) {
	resolved := 0
	transport := &authRoundTripper{
		resolveBearer: func(context.Context) (string, error) {
			resolved++
			return "access-token-" + strconv.Itoa(resolved), nil
		},
		base: roundTripFunc(func(req *http.Request) (*http.Response, error) {
			want := "Bearer access-token-" + strconv.Itoa(resolved)
	...
	for range 2 {
		req, _ := http.NewRequest(http.MethodPost, "http://mcp.example", nil)
		if _, err := transport.RoundTrip(req); err != nil {
			t.Fatal(err)
		}
	}
	if resolved != 2 {
		t.Fatalf("bearer resolved %d times", resolved)
	}
}
