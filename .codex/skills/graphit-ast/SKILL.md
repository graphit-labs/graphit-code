...
For a request such as changing order validation: locate the definition → read relevant branches → inspect callers and test candidates → record affected contracts, files, dependencies and acceptance checks in Task before implementation. Use Knowledge for intended behavior and decisions, AST for current behavior, cluster/Hub routing for ecosystem project identity, and Memory only for reusable non-obvious findings. Load each additional skill immediately before its module is needed; do not preload every module.
