...
> **MCP only — never the CLI.** All memory operations below go through the memory MCP tools.

- List ALL project memories by calling the `graphit_memory_list` MCP tool (or `graphit_memory_search` with a keyword).
- For each memory, ask: **Is this still accurate given what I just did?**
- If a memory is now outdated or contradicted by your changes, **update it immediately**: call `graphit_memory_delete` (with the old `id`), then `graphit_memory_insert` (with the corrected `title`, `type`, and `content`).
...
For each new memory, call the `graphit_memory_insert` MCP tool with a descriptive `title`, the appropriate `type`, and a detailed `content` explaining the insight, context, and reasoning (MCP only — never the CLI):
...
Resolve the correct path for the current IDE by calling the `graphit_hub_type-path` MCP tool — **never the CLI**.

**Step 3a: Resolve the artifact path**

Call the `graphit_hub_type-path` MCP tool (passing absolute `project_dir`, the artifact `type`, and the `name`) to get the exact path where the artifact should live. Examples of what it returns:
...
graphit_hub_type-path(project_dir: "/path/to/project", type: "rule", name: "no-direct-db-access")
  → /path/to/project/.agents/rules/no-direct-db-access.md

graphit_hub_type-path(project_dir: "/path/to/project", type: "command", name: "pre-deploy-check")
  → /path/to/project/.agents/workflows/pre-deploy-check.md
```

**Step 3b: Create the artifact content**

Take the path returned by `graphit_hub_type-path` and create the file using your IDE's native file-creation/editing tools — do NOT shell out with `cat`, `echo`, or `mkdir`.

For **file-based types** (rule, command, agent) — write the content directly to the returned path (e.g. `.agents/rules/no-direct-db-access.md`):
...
call the `graphit_hub_submit` MCP tool with the artifact `id`, `local_path`, `type`, and `version` (e.g. version 1.0.0).
```
...
To manually publish/submit updates to the remote Hub, call the `graphit_hub_submit` MCP tool with the artifact `id`, `local_path`, `type`, and a new `version`.
