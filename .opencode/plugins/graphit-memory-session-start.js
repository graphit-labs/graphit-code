...
export const GraphitLifecycle = async ({ directory }) => {
  const invariant = "Graphit invariant: when a Graphit skill and MCP tool cover the current action, use them before native equivalents and load only that skill, once, at the moment it is needed. If the required Graphit tool is unavailable in this agent, continue with its default native tools. Do not substitute the Graphit CLI for MCP."
  const loadBootstrap = () => {
    let bootstrap = "Graphit invariant: when a Graphit skill and MCP tool cover the current action, use them before native equivalents and load only that skill, once, at the moment it is needed. If the required Graphit tool is unavailable in this agent, continue with its default native tools. Do not substitute the Graphit CLI for MCP.\nGraphit session bootstrap:\n1. Call `graphit_memory_mandatory` once and consume every result before acting.\n2. For the current request, call `graphit_memory_search` with `exclude_mandatory: true` and a focused query.\n3. Search returns titles. Read only the relevant result(s) with `graphit_wiki_source` and `wiki: \"memory\"` before acting."
    try {
